...
- `Manual Walk Path` 升级为 `Walk Guided Coverage / 行走引导全覆盖`：行走记录只作为经人工验证的可达空间种子，不再直接生成最终采集样条。
- 所有示教种子向下投影到真实 Mesh 地面并取可达域并集；相邻示教种子可恢复真实门洞和楼梯连接，但仍须通过 Mesh 墙体、地面与高度验证，不能因穿墙记录制造假 Portal。
- 可达房间改用 Boustrophedon 风格 Sweep Lane 和边界覆盖，按路径空间覆盖率自动补线，默认目标至少 98%；新增可达网格、覆盖网格、未覆盖网格和 `Path Spatial Coverage` 统计。
- 修复示教/自动 Curve 被 Blender Scene Ray Cast 误判为地面、天花或墙体的问题；可达空间规划的物理探测现在只接受 Mesh 几何。
- 覆盖候选评分使用局部栅格增量索引，避免大房间在补线阶段出现平方级耗时；最终统计仍使用精确几何距离检测。
- Blender 5.1.1 验收覆盖真实门洞、封闭上方空腔、全房间覆盖和 3 层实际输出。

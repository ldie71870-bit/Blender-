...
from . import path_planner_3d
from . import manual_walk_path
...
FLOORPLAN_SPACE_MODE_ITEMS = (
    ("REACHABLE", "从种子可达空间", "Only create curves in the 3D island reachable from the selected seed."),
    ("ALL", "全部室内空间", "Create curves in every detected indoor island."),
)

FLOORPLAN_SEED_MODE_ITEMS = (
    ("CURSOR", "3D 光标", "保留从 3D 光标可达的完整空间。"),
    ("VIEWPORT", "当前视角", "保留从当前 3D 视图观察位置可达的完整空间。"),
    ("OBJECT", "种子对象", "保留从指定对象位置可达的完整空间。"),
    ("CAMERA", "场景相机", "保留从场景相机位置可达的完整空间。"),
    ("ALL", "全部空间", "为每个检测到的室内连通空间生成曲线。"),
)
...
_RENDER_SIG_KEYS = (
    ...
    "cycles_samples", "cycles_denoise", "cycles_device", "camera_model", "color_look",
...
def scientific_realization_mode(settings):
    ...
    mode = getattr(settings, "scientific_realization_mode", "SCIENTIFIC_POSE_SEQUENCE")
    if mode not in {
        "LEGACY_CUBEMAP_OBJECTS", "SCIENTIFIC_CAMERA_OBJECTS", "SCIENTIFIC_POSE_SEQUENCE"
    }:
        return "SCIENTIFIC_POSE_SEQUENCE"
    return mode
...
def _floorplan_vertical_hits(scene, depsgraph, x, y, low_z, high_z, epsilon):
    """Return every upward-facing surface at an XY, highest first."""
    result = []
    cursor = high_z + epsilon
    guard = 0
    while cursor > low_z - epsilon and guard < 128:
        ...
        if not hit:
            break
        if normal is not None and normal.z > 0.05:
            if not result or abs(result[-1][0].z - location.z) > epsilon * 2.0:
                support_id = ""
                if obj is not None:
                    support = getattr(obj, "original", obj)
                    support_id = str(getattr(support, "name_full", getattr(support, "name", "")))
                result.append((location.copy(), normal.copy(), support_id))
        cursor = location.z - epsilon
        guard += 1
    return result


def _floorplan_probe_cells_3d(scene, settings, bounds):
    """Rasterize all legal floor/ceiling spans, including repeated XY layers."""
    mins, maxs = bounds
    depsgraph = bpy.context.evaluated_depsgraph_get()
    scale = float(getattr(scene.unit_settings, "scale_length", 1.0) or 1.0)
    units_per_meter = 1.0 / max(1e-9, scale)
    spacing = max(0.10, min(
        float(getattr(settings, "floorplan_probe_spacing", 0.60)),
        1.0 / max(0.05, float(getattr(settings, "floorplan_curve_density", 1.60))),
    )) * units_per_meter
    margin = max(0.05, min(
        float(getattr(settings, "floorplan_wall_margin", 0.50)),
        float(getattr(settings, "floorplan_narrow_margin", 0.12)),
    )) * units_per_meter
    headroom = max(0.50, float(getattr(settings, "floorplan_min_headroom", 1.0))) * units_per_meter
    eye_height = max(0.20, float(getattr(settings, "eye_height", 1.60))) * units_per_meter
    safety_radius = max(
        margin,
    ...
    hdirs = [Vector((math.cos(index * math.tau / 24.0), math.sin(index * math.tau / 24.0), 0.0))
             for index in range(24)]
    nx = max(1, int(math.ceil((maxs.x - mins.x) / spacing)))
    ny = max(1, int(math.ceil((maxs.y - mins.y) / spacing)))
    cells = {}
    clearance_search = max(3.0 * units_per_meter, margin * 6.0)
    vertical_distance = max(epsilon, maxs.z - mins.z + 2.0 * units_per_meter)
    for iy in range(ny + 1):
        y = min(maxs.y, mins.y + iy * spacing)
        for ix in range(nx + 1):
            x = min(maxs.x, mins.x + ix * spacing)
            surfaces = _floorplan_vertical_hits(
                scene, depsgraph, x, y, mins.z - units_per_meter, maxs.z + units_per_meter, epsilon
            )
            for floor_location, floor_normal, support_id in surfaces:
                ...
                if not ceiling_hit or ceiling_location.z - floor_location.z < headroom:
                    continue
                camera_z = min(
                    max(floor_location.z + eye_height, floor_location.z + margin),
                    ceiling_location.z - margin,
                )
                point = Vector((x, y, camera_z))
...
def _floorplan_segment_clear_3d(
    scene, depsgraph, left, right, margin, hdirs, eye_height, units_per_meter,
    maximum_step_m,
):
    delta = right - left
    ...
    if hit:
        return False
    step = max(0.08 * units_per_meter, min(0.25 * units_per_meter, distance * 0.25))
    ...
    for index in range(count + 1):
        point = left.lerp(right, index / count)
        ...
        if abs(floor_location.z - expected_floor) > support_tolerance:
            return False
        # Endpoint cells have already passed the full floor/ceiling headroom
        # probe. Requiring another ceiling hit within an arbitrary 3 m here
        # incorrectly disconnects atriums and tall rooms.
    return True


def _floorplan_graph_edge_clear_3d(
    scene, depsgraph, left, right, eye_height, units_per_meter, maximum_step_m,
):
    """Fast physical adjacency check used while building the large cell graph."""
    delta = right - left
    distance = delta.length
    if distance <= 1e-8:
        return True
    ...
    return bool(
        floor_hit
        and abs(floor_location.z - (midpoint.z - eye_height)) <= support_tolerance
    )


def _clear_multilevel_path_debug(scene):
    collection = bpy.data.collections.get("GS_SCIENTIFIC_PATH_DEBUG")
    if collection is None:
        return
    for obj in list(collection.objects):
        if obj.get("gs_path_planner_debug"):
            bpy.data.objects.remove(obj, do_unlink=True)


def _path_debug_curve(collection, name, points, color, category):
    if len(points) < 2:
        return None
    ...
    obj["gs_path_planner_debug"] = True
    obj["gs_path_debug_category"] = category
    collection.objects.link(obj)
    return obj


def _path_debug_points(collection, name, points, color, category):
    if not points:
        return None
...
def _build_multilevel_debug(scene, result, cells, enabled):
    ...
    if collection is None:
        collection = bpy.data.collections.new("GS_SCIENTIFIC_PATH_DEBUG")
        scene.collection.children.link(collection)
    reachable_keys = set(result.graph)
    all_keys = set(cells)
    _path_debug_points(
        collection, "GS_AllWalkableCells",
        [(cells[key].x, cells[key].y, cells[key].floor_z + 0.025) for key in sorted(all_keys)],
        (0.35, 0.55, 0.95, 1.0), "ALL_WALKABLE",
    )
    _path_debug_points(
        collection, "GS_ReachableCells",
        [(cells[key].x, cells[key].y, cells[key].floor_z + 0.035) for key in sorted(reachable_keys)],
        (0.10, 0.90, 0.35, 1.0), "REACHABLE",
    )
    _path_debug_points(
        collection, "GS_RejectedCells",
        [(cells[key].x, cells[key].y, cells[key].floor_z + 0.045)
         for key in sorted(all_keys - reachable_keys)],
        (0.95, 0.12, 0.08, 1.0), "REJECTED",
    )
    seed_key = tuple(result.stats.get("reachable_seed_key") or ())
    if seed_key in cells:
        seed = cells[seed_key]
        _path_debug_points(
            collection, "GS_ReachabilitySeed",
            [(seed.x, seed.y, seed.floor_z + 0.08)],
            (1.0, 0.80, 0.05, 1.0), "SEED",
        )
    for index, region in enumerate(result.regions):
        mesh = bpy.data.meshes.new(f"GS_FloorRegion_{index:02d}")
        mesh.from_pydata(
            [(cells[key].x, cells[key].y, cells[key].floor_z + 0.02)
             for key in region.cell_keys], [], []
        )
        obj = bpy.data.objects.new(f"GS_FloorRegion_{index:02d}", mesh)
        obj.display_type = "WIRE"
        obj.hide_render = True
        obj["gs_scientific_debug"] = True
        obj["gs_path_planner_debug"] = True
    ...
    for index, fragment in enumerate(result.final_fragments):
        _path_debug_curve(collection, f"GS_FinalSpline_{index:03d}", fragment.points,
                          (0.10, 0.90, 0.35, 1.0), "FINAL_SPLINE")
    for index, connector in enumerate(result.connectors):
        _path_debug_curve(collection, f"GS_{connector.kind}_{index:02d}", connector.points,
...
def _build_scientific_floorplan_network_3d(scene, settings, bounds):
    ...
    if not cells:
        return None
    depsgraph = bpy.context.evaluated_depsgraph_get()
    config = path_planner_3d.PlannerConfig(
        grid_spacing=spacing,
        units_per_meter=units_per_meter,
        floor_tolerance_m=float(getattr(settings, "multilevel_floor_tolerance", 0.10)),
        maximum_connector_step_m=float(getattr(settings, "multilevel_maximum_step", 0.38)),
        minimum_floor_cells=max(2, int(getattr(settings, "multilevel_minimum_floor_cells", 6))),
        stitch_distance_m=float(getattr(settings, "fragment_stitch_distance", 1.25)),
        stitch_angle_degrees=float(getattr(settings, "fragment_stitch_angle", 65.0)),
        smoothing_iterations=(
            int(getattr(settings, "curve_smoothing_iterations", 2))
            if bool(getattr(settings, "curve_smoothing", True)) else 0
        ),
    ...
    planner_cells = [path_planner_3d.WalkableCell(
        key=cell.key,
        point=(cell.x, cell.y, cell.floor_z + eye_height),
    ...
    def edge_valid(left, right):
        return _floorplan_graph_edge_clear_3d(
            scene, depsgraph, Vector(left), Vector(right), eye_height,
    ...
    if restrict_to_reachable:
        seed_key = _floorplan_seed_cell_3d(
            scene, settings, cells, spacing, units_per_meter
        )
        if seed_key is None:
            seed_name = {
                "CURSOR": "3D 光标",
                "OBJECT": "种子对象",
                "CAMERA": "场景相机",
            }.get(getattr(settings, "floorplan_seed_mode", "CURSOR"), "可达性种子")
            raise RuntimeError(
                "No reachable walkable surface near planning seed. "
                f"{seed_name}附近没有合法可行走地面。请把它放到目标房间内，"
                "或将“空间范围”改为“全部室内空间”。"
            )

    result = path_planner_3d.plan_walkable_paths(
        planner_cells, config,
...
def _floorplan_seed_location(scene, settings):
    ...
    if mode == "VIEWPORT":
        ...
        if getattr(space, "type", None) == "VIEW_3D" and getattr(space, "region_3d", None):
            return space.region_3d.view_matrix.inverted().translation.copy()
        screen = getattr(bpy.context, "screen", None)
        for area in getattr(screen, "areas", ()) if screen else ():
            if area.type == "VIEW_3D":
                return area.spaces.active.region_3d.view_matrix.inverted().translation.copy()
    return scene.cursor.location.copy()


def _floorplan_seed_cell_3d(scene, settings, cells, spacing, units_per_meter):
    """Resolve the user seed to a legal cell in the same vertical space."""
    seed = _floorplan_seed_location(scene, settings)
    horizontal_limit = max(2.0 * units_per_meter, spacing * 3.0)
    vertical_limit = max(1.0 * units_per_meter, spacing * 2.0)
    depsgraph = bpy.context.evaluated_depsgraph_get()
    seed_hit, seed_floor, seed_normal, _face, seed_object, _matrix = scene.ray_cast(
        depsgraph,
        seed + Vector((0.0, 0.0, 0.05 * units_per_meter)),
        Vector((0.0, 0.0, -1.0)),
        distance=max(vertical_limit + 0.10 * units_per_meter, 4.0 * units_per_meter),
    )
    if seed_hit and seed_normal is not None and seed_normal.z > 0.05:
        support = getattr(seed_object, "original", seed_object) if seed_object is not None else None
        support_id = str(getattr(support, "name_full", getattr(support, "name", ""))) if support else ""
        exact = []
        for key, cell in cells.items():
            if support_id and cell.support_id and cell.support_id != support_id:
                continue
            horizontal = math.hypot(cell.x - seed.x, cell.y - seed.y)
            floor_gap = abs(cell.floor_z - seed_floor.z)
            if horizontal <= spacing * 1.75 and floor_gap <= max(0.10 * units_per_meter, spacing * 0.60):
                exact.append((horizontal, floor_gap, key))
        if exact:
            return min(exact)[2]
    best = None
    for key, cell in cells.items():
        ...
        if horizontal > horizontal_limit:
            continue
        if seed.z < cell.floor_z:
            vertical_gap = cell.floor_z - seed.z
        elif seed.z > cell.ceiling_z:
            vertical_gap = seed.z - cell.ceiling_z
        else:
            vertical_gap = 0.0
        if vertical_gap > vertical_limit:
            continue
        # A cursor anywhere between floor and ceiling selects this storey. The
        # horizontal and floor distances then make the choice deterministic.
        score = (vertical_gap, horizontal, abs(seed.z - cell.floor_z), key)
...
def _create_floorplan_curve(collection, name, points, allow_3d=False):
    if len(points) < 2:
        return None
    for a, b in zip(points, points[1:]):
        dz = abs(b.z - a.z)
        if not allow_3d and dz > 1e-4:
            return None
...
def build_floorplan_path(scene, settings):
    ...
    if not bounds:
        return None
    if bool(getattr(settings, "multilevel_planning", True)):
        result = _build_scientific_floorplan_network_3d(scene, settings, bounds)
        return result
    depsgraph = bpy.context.evaluated_depsgraph_get()
...
def activate_cycles_devices(restrict_names=None, prefer_backend=None):
    ...
    if not gpus:
        return ("CPU", [])
    if prefer_backend and any(b == prefer_backend for b, _ in gpus):
...
SETTINGS_VALUE_KEYS = (
    ...
    "curve_smoothing",
    "fragment_stitching",
    "multilevel_floor_tolerance",
    "multilevel_maximum_step",
    "multilevel_maximum_slope",
    "multilevel_minimum_floor_cells",
    "fragment_stitch_distance",
    "fragment_stitch_angle",
    "curve_smoothing_iterations",
    "multilevel_large_room_area",
    "multilevel_coverage_paths",
...
try:
    ...
    result = dict(control)
    result.update(count=count, point_count=point_count, error="")
...
def launch_background_render(context, settings, patch_only=False):
    ...
    try:
        output_plan = _resolved_output_plan(settings)
        if output_plan.needs_object_groups:
...
_MANUAL_WALK_UPDATE_LOCK = False


def _manual_walk_regenerate_from_update(self, context):
    if _MANUAL_WALK_UPDATE_LOCK or not context or not getattr(context, "scene", None):
        return
    try:
        manual_walk_path.regenerate_manual_layers(context.scene, self)
    except Exception:
        pass


def update_manual_walk_layout(self, context):
    global _MANUAL_WALK_UPDATE_LOCK
    if _MANUAL_WALK_UPDATE_LOCK:
        return
    _MANUAL_WALK_UPDATE_LOCK = True
    try:
        count = int(getattr(self, "manual_walk_layer_count", "3"))
        offsets = manual_walk_path.default_layer_offsets(
            count, float(getattr(self, "manual_walk_layer_spacing", 0.35))
        )
        for index in range(1, 5):
            value = offsets[index - 1] if index <= len(offsets) else 0.0
            setattr(self, f"manual_walk_layer_{index}_offset", value)
    finally:
        _MANUAL_WALK_UPDATE_LOCK = False
    _manual_walk_regenerate_from_update(self, context)


def update_manual_walk_collision(self, context):
    _manual_walk_regenerate_from_update(self, context)
...
class GSCOLMAP_Settings(PropertyGroup):
    ...
    scientific_origin_mode: EnumProperty(name="Origin Source", items=SCIENTIFIC_ORIGIN_MODE_ITEMS, default="AUTO_GRID_PATH")
    multilevel_planning: BoolProperty(
        name="3D Multi-Level Planning", default=True,
        description="Detect repeated XY floors, stairs, landings and ramps and build continuous XYZ paths.",
    )
    curve_smoothing: BoolProperty(
        name="Curve Smoothing", default=True,
        description="Smooth centerline paths, accepting a smoothed segment only after collision and support validation.",
    )
    fragment_stitching: BoolProperty(
        name="Fragment Stitching", default=True,
        description="Merge compatible fragments using distance, tangent, floor-region and collision checks.",
    )
    multilevel_floor_tolerance: FloatProperty(name="Flat Floor Tolerance (m)", default=0.10, min=0.02, max=0.30)
    multilevel_maximum_step: FloatProperty(name="Maximum Stair Step (m)", default=0.38, min=0.10, max=0.80)
    multilevel_maximum_slope: FloatProperty(name="Maximum Walkable Slope", default=42.0, min=5.0, max=70.0)
    multilevel_minimum_floor_cells: IntProperty(name="Minimum Floor Region Cells", default=6, min=2, max=500)
    fragment_stitch_distance: FloatProperty(name="Maximum Stitch Distance (m)", default=1.25, min=0.05, max=5.0)
    fragment_stitch_angle: FloatProperty(name="Maximum Stitch Turn", default=65.0, min=5.0, max=120.0)
    curve_smoothing_iterations: IntProperty(name="Smoothing Passes", default=2, min=0, max=5)
    multilevel_large_room_area: FloatProperty(name="Large Room Area (m2)", default=28.0, min=4.0, max=500.0)
    multilevel_coverage_paths: IntProperty(name="Large Room Offset Paths", default=2, min=0, max=6)
    ...
    manual_walk_layer_count: EnumProperty(
        ...
        items=(("2", "2 Layers", "Upper / Lower"),
               ("3", "3 Layers", "Upper / Middle / Lower（推荐）"),
               ("4", "4 Layers", "Upper / Upper Middle / Lower Middle / Lower")),
        default="3",
        update=update_manual_walk_layout,
    )
    manual_walk_layer_spacing: FloatProperty(
        name="Layer Spacing (m)", default=0.35, min=0.05, max=2.0,
        description="修改后从未改变的 BasePath 立即重新生成各高度层。",
        update=update_manual_walk_layout,
    )
    manual_walk_layer_1_offset: FloatProperty(name="Layer 1 Z Offset (m)", default=0.35, min=-3.0, max=3.0, update=update_manual_walk_collision)
    manual_walk_layer_2_offset: FloatProperty(name="Layer 2 Z Offset (m)", default=0.0, min=-3.0, max=3.0, update=update_manual_walk_collision)
    manual_walk_layer_3_offset: FloatProperty(name="Layer 3 Z Offset (m)", default=-0.35, min=-3.0, max=3.0, update=update_manual_walk_collision)
    manual_walk_layer_4_offset: FloatProperty(name="Layer 4 Z Offset (m)", default=-0.525, min=-3.0, max=3.0, update=update_manual_walk_collision)
    manual_walk_collision_cut: BoolProperty(
        name="Collision Cut", default=True,
        description="对 Offset 后的整条样条密集采样；碰撞区间合并、留安全边距并自动断开。",
        update=update_manual_walk_collision,
    )
    manual_walk_safety_radius: FloatProperty(
        name="Path Safety Radius (m)", default=0.20, min=0.05, max=1.0,
        description="相机安全球半径，同时作为碰撞区间前后的裁切余量。",
        update=update_manual_walk_collision,
    )
    manual_walk_advanced: BoolProperty(name="Manual Walk Advanced", default=False)
    ...
    scientific_realization_mode: EnumProperty(name="Realization Backend", items=SCIENTIFIC_REALIZATION_MODE_ITEMS, default="SCIENTIFIC_POSE_SEQUENCE")
...
class GSCOLMAP_OT_auto_floorplan_path(Operator):
    ...
    def execute(self, context):
        ...
        settings.rig_mode = "PATH"
        self.report({"INFO"}, f"井字排线已生成：{len(result['objects'])} 条曲线，{result['points']} 个点。接着点击创建相机阵列。")
        return {"FINISHED"}
...
def draw_gs_colmap_panel(self, context):
    ...
    try:
        ...
        if settings.rig_mode == "PATH":
            ...
            walk.prop(settings, "manual_walk_layer_count", text="Layer Count")
            walk.prop(settings, "manual_walk_layer_spacing", text="Layer Spacing")
            walk.prop(settings, "manual_walk_collision_cut", text="Collision Cut")
            if settings.manual_walk_collision_cut:
            ...
            advanced_walk.prop(
                settings, "manual_walk_advanced", text="",
                icon="TRIA_DOWN" if settings.manual_walk_advanced else "TRIA_RIGHT",
                emboss=False,
            )
            advanced_walk.label(text="Advanced Layer Offsets")
            if settings.manual_walk_advanced:
                for offset_index in range(1, int(settings.manual_walk_layer_count) + 1):
                    walk.prop(
                        settings, f"manual_walk_layer_{offset_index}_offset",
                        text=f"Layer {offset_index} Z Offset",
                    )
            ...
            if settings.path_capture_mode == "SCIENTIFIC_THREE_LAYER":
                ...
                science.prop(settings, "multilevel_planning", text="3D Multi-Level Planning")
                science.prop(settings, "curve_smoothing", text="Curve Smoothing")
                science.prop(settings, "fragment_stitching", text="Fragment Stitching")
                ...
                if settings.scientific_advanced_expand:
                    if settings.multilevel_planning:
                        science.prop(settings, "multilevel_floor_tolerance", text="平层高度容差")
                        science.prop(settings, "multilevel_maximum_step", text="最大楼梯步高")
                        science.prop(settings, "multilevel_maximum_slope", text="最大可行走坡度")
                        science.prop(settings, "multilevel_minimum_floor_cells", text="最小楼层区域网格")
                        science.prop(settings, "multilevel_large_room_area", text="大空间面积阈值")
                        science.prop(settings, "multilevel_coverage_paths", text="大空间偏移路径数")
                    if settings.curve_smoothing:
                        science.prop(settings, "curve_smoothing_iterations", text="平滑次数")
                    if settings.fragment_stitching:
                        science.prop(settings, "fragment_stitch_distance", text="最大拼接距离")
                        science.prop(settings, "fragment_stitch_angle", text="最大拼接转角")
            ...
            route_head.label(text="室内GS训练井字排线", icon="CURVE_PATH")
            if settings.floorplan_expand:
                route_box.prop(settings, "floorplan_space_mode", text="空间范围")
                if settings.floorplan_space_mode == "REACHABLE":
                    route_box.prop(settings, "floorplan_seed_mode", text="可达性起点")
                    if settings.floorplan_seed_mode == "OBJECT":
                        route_box.prop(settings, "floorplan_seed_object", text="种子对象")
                ...
                if settings.floorplan_layer_mode in {"THREE", "FOUR"}:
                ...
                route_box.operator("gs_colmap.auto_floorplan_path", text="生成井字排线", icon="MOD_CURVE")
        else:
            box.prop(settings, "target_object", text=tr(settings, "target"))
            box.prop(settings, "camera_count", text=tr(settings, "count"), slider=True)

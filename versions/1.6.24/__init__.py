    ...
    try:
        ...
        if settings.rig_mode == "PATH":
            ...
            if settings.path_capture_mode == "LEGACY_PANORAMA_CUBE":
                ...
                detail_recommended = sum(max(1, int(item['object'].get('gs_detail_min_samples', 1))) for item in path_items)
                if detail_recommended > requested_station_count:
                    budget.label(text=f"按设定 {requested_station_count} 个站点分配；细部建议 {detail_recommended}，未额外增加", icon="INFO")
                budget.label(
                    text=(

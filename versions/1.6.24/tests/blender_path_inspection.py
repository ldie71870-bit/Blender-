...
assert abs(cameras[0].data.display_size - 0.01) < 1e-6

addon.invalidate_camera_overlay(scene)
points,lines=addon._path_inspection_geometry(scene,settings)
assert len(points)==3, "Normal view must show all generated cameras without isolation"
cameras[0].location.x=7.0
bpy.context.view_layer.update()
points,lines=addon._path_inspection_geometry(scene,settings)
assert any(abs(p[0]-7.)<1e-5 for p in points), "Moving cameras must invalidate overlay cache"
removed=cameras.pop()
bpy.data.objects.remove(removed,do_unlink=True)
bpy.context.view_layer.update()
assert len(addon._path_inspection_geometry(scene,settings)[0])==2, "Deleted cameras must leave the overlay"
settings.camera_overlay_enabled=False
assert cameras[0].data.display_size==0.5, "Disabling the global proxy must restore native size"

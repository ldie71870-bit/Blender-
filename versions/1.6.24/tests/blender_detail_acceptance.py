...
s.shell_surface_detail_enabled=False
...
# Surface-aware mode must derive targeted views from actual depth/object boundaries.
s.shell_surface_detail_enabled=True;s.shell_surface_detail_cameras=4
surface_collection=bpy.data.collections.new('ShellSurfaceAware');scene.collection.children.link(surface_collection)
surface_group,_,_,_,_=addon._generate_spherical_shell_12_group(
    scene,s,surface_collection,(1.2,1.2,1.0),(0,1,0),2,visible_route,bpy.context.evaluated_depsgraph_get())
surface_views=[camera for camera in surface_group if str(camera.get('shell_view_kind','')).endswith('SURFACE_EDGE')]
assert surface_views,(len(surface_group),[camera.get('shell_view_kind') for camera in surface_group])
assert all(camera.get('shell_target_object') for camera in surface_views)

...
# 1.6.9 lacks fraction; progress is deliberately removed too so introducing
# another new method on the same stale module cannot accidentally pass.
saved_optional_api = {}
for attr in ('fraction', 'progress'):
    if hasattr(jobs, attr):
        saved_optional_api[attr] = getattr(jobs, attr)
        delattr(jobs, attr)

try:
    assert not hasattr(jobs, 'fraction') and not hasattr(jobs, 'progress')
    ...
    for attr, value in saved_optional_api.items():
        setattr(jobs, attr, value)
    jobs.shutdown()

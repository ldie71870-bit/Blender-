...
def gap_lines(main_routes,models,probe):
    ...
    for mi,model in enumerate(models):
        ...
        model['gap_missing']=missing
        model['gap_origin']=origin
        for line in gap_routing.pocket_routes(cells,graph,missing,origin,probe.step,model['valid']):
            line['model']=mi
            line['space_targets']={(mi,*k) for k in line.pop('space_keys')}
            lines.append(line)
    return lines
...
def refine(main_routes, models, probe, settings, progress):
    ...
    main_sample_spacing=max(.30,sum(planner.length(r.points) for _,_,r in main_routes)/max(1,int(settings.camera_count)))
    sample_points=[]
    samples_by_layer=defaultdict(list)
    seen=set()
    for layer_index,_,route in main_routes:
        for point in resample(route.points,main_sample_spacing):
    ...
    for instance in probe.depsgraph.object_instances:
        obj=instance.object
        if obj.type!='MESH' or region_volume.is_region(obj) or obj.get('gs_camera_mesh_visual'):continue
    ...
    # Spatial pockets remain useful even with no sampled surface targets.
    observations={}
    visible_cache={}
            ...
            for half_length in (.60,.45,.30,.225,.15):
                ...
                if best is None or score>best[0]:
                    best=(score,dict(points=[a,b],length=half_length*2,observations=views,
                                     model=candidate['model'],center=center,kind='DETAIL'))
                # Prefer a useful long baseline; shortening only helps when a
                # longer segment was geometrically blocked.
                break
        if best is not None and best[0]>0:
            line=best[1]
            missing_space=model.get('gap_missing',set())
            if missing_space:
                line['space_targets']={(candidate['model'],*k) for k in planner.covered_keys(
                    [planner.Route(line['points'])],model['cells'],model['graph'],.20,model['gap_origin'],probe.step)&missing_space}
    ...
    # Reserve half of each layer's allocation for real spatial gaps. Surface
    # detail cannot outbid all coherent pocket routes with many tiny targets.
    reserve=max(2,math.ceil(int(settings.contour_detail_budget)/max(1,len(models))*.5))
    selected,before,after=metrics.select_lines(targets,observations,line_candidates,
                                               int(settings.contour_detail_budget),baseline,angle,balanced=True,reserve_gaps=reserve)
    result=[]
    details=[]
    for candidate in selected:
        ...
        details.append(dict(layer=model['label'],new_surface_cells=candidate['gain'],
                            ...
                            pocket=candidate.get('pocket'),pass_index=candidate.get('pass_index')))
    layer_reports=[]
    ...
    report=dict(target_count=len(targets),main_observed_cells=len(before),final_observed_cells=len(after),
                main_surface_ratio=len(before)/max(1,len(targets)),final_surface_ratio=len(after)/max(1,len(targets)),
                ...
                new_gap_cells=sum(c.get('space_gain',0) for c in selected),reserved_gap_lines_per_layer=reserve,
                main_observation_spacing_m=main_sample_spacing,

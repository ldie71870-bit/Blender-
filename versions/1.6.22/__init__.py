...
def _capture_ray_cast(scene, depsgraph, origin, direction, distance):
    """Skip generated display geometry, continuing to real walls behind it."""
    origin=Vector(origin);direction=Vector(direction).normalized()
    remaining=float(distance)
    epsilon=1e-5*_scene_units_per_meter(scene)
    for _ in range(64):
        result=scene.ray_cast(depsgraph,origin,direction,distance=max(0.,remaining))
        hit,point,normal,face,obj,matrix=result
        if not hit or obj is None:return result
        original=getattr(obj,'original',obj)
        display_geometry=(is_camera_mesh_visual(obj) or region_volume.is_region(obj) or original.get('gs_contour_version') or
                          (original.type=='CURVE' and original.get('gs_route_role')))
        if not display_geometry:return result
        remaining-=(point-origin).length+epsilon
        if remaining<=0:return False,point,normal,-1,None,matrix
        origin=point+direction*epsilon
    raise RuntimeError('Too many overlapping path displays during camera clearance check')
...
def path_component_counts(settings, components):
    ...
    if sum(counts) < sum(minimums):
        # Detail routes are generated only when they can support enough distinct
        # viewpoints.  Treat those minimums as a lower bound for the effective
        # station budget instead of failing an otherwise valid camera build.
        return minimums
...
def sample_path_components_with_regions(settings, components):
    ...
    for item, count in zip(components, path_component_counts(settings, components)):
        for point, tangent in sample_polyline_evenly(item["points"], count):
            if not any(point_in_station_region(point, region) for region in regions):
                sampled.append((point, tangent, item["object"]))

    for region in regions:
        fragments = []
        for component in components:
            fragments.extend(_component_region_fragments(component, region))
        if not fragments:
            continue
        for fragment, count in zip(
            fragments, distribute_counts_by_length(region.station_count, fragments)
        ):
            for point, tangent in _sample_polyline_centred(fragment["points"], count):
                sampled.append((point, tangent, fragment["object"]))

    # Preserve the established minimum viewpoints for every short detail line.
    by_object = {}
    for point, tangent, obj in sampled:
        by_object.setdefault(obj.name, []).append(point)
    for component in components:
        obj = component["object"]
        minimum = max(1, int(obj.get("gs_detail_min_samples", 1)))
        existing = by_object.setdefault(obj.name, [])
        missing = minimum - len(existing)
        if missing <= 0:
            continue
        candidates = _sample_polyline_centred(component["points"], max(minimum * 3, 8))
        candidates.sort(key=lambda item: any(point_in_station_region(item[0], region) for region in regions))
        for point, tangent in candidates:
            if all((point - prior).length > 1e-5 for prior in existing):
                sampled.append((point, tangent, obj))
    ...
    try:
        ...
        if settings.rig_mode == "PATH":
            ...
            if settings.path_capture_mode == "LEGACY_PANORAMA_CUBE":
                ...
                if station_count > requested_station_count:
                    budget.label(
                        text=f"细部线最低站点需求：已由 {requested_station_count} 自动扩展到 {station_count}",
                        icon="INFO",
                    )
                budget.label(
                    text=(
...
def unregister():
    coverage_heat.unregister()
    global _CAMERA_OVERLAY_HANDLE, _STATION_PLAN_IMAGE_HANDLE
    if bpy.app.timers.is_registered(initialize_registered_scene_state):
        bpy.app.timers.unregister(initialize_registered_scene_state)
    for scene in tuple(getattr(bpy.data,'scenes',())):
        _restore_path_inspection(scene)
        _restore_station_plan_view(scene)
        sync_native_camera_sizes(scene,False)
    for keymap, item in _PATH_INSPECTION_KEYMAPS:
        try:
            keymap.keymap_items.remove(item)
        except Exception:
            pass
    _PATH_INSPECTION_KEYMAPS.clear()
    if _CAMERA_OVERLAY_HANDLE is not None:
        try:
            bpy.types.SpaceView3D.draw_handler_remove(_CAMERA_OVERLAY_HANDLE, "WINDOW")
        except Exception:
            pass
        _CAMERA_OVERLAY_HANDLE = None
    if _STATION_PLAN_IMAGE_HANDLE is not None:
        try:
            bpy.types.SpaceImageEditor.draw_handler_remove(_STATION_PLAN_IMAGE_HANDLE, "WINDOW")
        except Exception:
            pass
        _STATION_PLAN_IMAGE_HANDLE = None
    if sync_camera_overlay_on_load in bpy.app.handlers.load_post:
        bpy.app.handlers.load_post.remove(sync_camera_overlay_on_load)
    if camera_overlay_scene_changed in bpy.app.handlers.depsgraph_update_post:
        bpy.app.handlers.depsgraph_update_post.remove(camera_overlay_scene_changed)
    invalidate_camera_overlay()
    invalidate_station_plan()
    ...
    if restore_transient_view_modes in bpy.app.handlers.load_pre:
        bpy.app.handlers.load_pre.remove(restore_transient_view_modes)
    if restore_transient_view_modes in bpy.app.handlers.save_pre:
        bpy.app.handlers.save_pre.remove(restore_transient_view_modes)
    if cleanup_legacy_camera_mesh_visuals in bpy.app.handlers.load_post:
        bpy.app.handlers.load_post.remove(cleanup_legacy_camera_mesh_visuals)
    try:

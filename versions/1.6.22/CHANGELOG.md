...
- 新增可选区域排线、当前视图创建线框 Box、选中、编辑形状和封闭性检查入口。
- 使用封闭网格真实内部限制主线和所有补线，支持凹形、旋转、负缩放及编辑加线，逐段检查跨边界情况。
- 采样范围只裁减 XY，仍从真实场景读取区域底部以下的支撑地面与上方天花板。
- 区域辅助物不作为路径或球壳相机障碍、不参与渲染和导出点云射线命中。
- 后台快照保留区域设置，生成失败或取消时保留已有预览。
- UI 版号在模块加载时保存，避免 Blender 扩展模式修改 `bl_info` 后退回 1.3.4。
- 延迟安装注册时的场景代理初始化，避免扩展安装器的 `_RestrictData` 环境导致启用失败。

## 1.6.8 - 2026-09-07

- 新增门洞、凹位与遮挡边缘的安全直线探入候选，以及现有路径集合的增量补线按钮。
- 根据现有相机实际视锥、镜头偏移、裁剪面、首个网格命中和 12 cm / 12 度视差审计分物体表面采样。
- 用实际球壳生成逻辑验证候选，淘汰无新增有效视角的短线，后续候选优先观察尚未补足的表面。
- 探入线最多观察四个实际目标，每目标使用三个横移或下移光心，保留原相机净距与自适应半径设置。
- 修复带厚度路径求值为网格后误入碰撞检测的问题，同时修复同步和分步球壳生成的路径自遮挡。
- 全局 GPU 相机代理默认开启，不依赖孤立路径；自动失效缓存，并恢复原生相机显示尺寸。
- 临时验证相机统一延迟清理，防止射线检测过程中依赖图对象被提前删除。

## 1.6.7 - 2026-09-07

- 在基础 12 方位之外新增可选的真实场景表面补视角，默认开启，普通路径每站最多追加 4 台。
- 每站向 24 个方位和三档浅俯仰发射网格射线，按深度突变、物体边界和开口远侧表面选择实际目标点。
- 为低位家具缝隙保留一半补拍预算，并对同一开口生成左右横移、降低光心的短基线相机对。
- 细部短线可比普通路径多获得 2 台补视角；所有新增位置继续使用自适应半径和表面净距过滤。
- 新增补视角开关、每站上限、探测距离和深度突变阈值 UI，并在报告中记录补充相机数量。
- 使用 `out2/transforms.json` 反解原训练的 250 个站点，在 Blender 场景中逐点复核截图对应的八个失败区域。

## 1.6.6 - 2026-09-07

- 新增默认关闭的“站点配额平面图”开关；关闭时完全沿用原路径采样算法。
- 从当前场景和路径范围临时正交渲染灰白 CAD 风格底图，并在独立图像编辑器中规划。
- 在底图上叠加路径、预计站点、配额矩形和区域数量标签，图片缩放和平移不改变世界坐标映射。
- 支持先输入目标站点数再直接拖框；框内常规采样由区域配额替换，框外保持原分配，可创建多个不重叠区域。
- 区域目标低于完整细部短线最低采样需求时保留安全下限，并在面板显示实际值与目标值。
- CAD 渲染完成后恢复原相机、渲染器、分辨率、世界背景和对象渲染可见性，不创建持久场景对象。

## 1.6.5 - 2026-09-05

- 新增单路径相机检查：选中曲线后通过 N 面板按钮或 `Alt + Shift + I` 隐藏其他路径，同时保留完整场景。
- 用批量 GPU 点和短朝向线显示当前路径上的相机，不创建代理对象、Mesh 或材质。
- 检查模式隐藏插件生成相机的原生大线框，支持点大小、朝向长度、穿透显示和手动刷新。
- 退出检查时恢复各路径及相机此前的隐藏状态和当前曲线的置顶状态。
- 移除绿色金字塔 Mesh 代理开关、参数、操作器和生成阶段，并自动清理旧版残留数据。

## 1.6.4 - 2026-09-05

- 修复细部线最低站点需求超过旧相机数量设置时，球壳任务在准备阶段中止且不产生相机的问题。
- 将每条细部线的最低采样数作为真实站点预算下限；旧设置不足时自动扩展，并在面板显示实际站点数与预计相机总数。
- 球壳安全检查改为逐台相机判定：只剔除贴近或穿过几何表面的方向，不再因一个方向无效而丢弃整个站点。
- 相邻站点默认绕竖直轴递增旋转 5°；面板新增可调的“每站旋转角度”和“球壳相机表面净距”。
- 全部站点均未通过球壳或回退安全检查时明确失败并保留旧相机阵列，不再以零相机完成提交。

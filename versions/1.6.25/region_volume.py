...
def region_enabled(settings):
    """Saved professional region preferences must not constrain simple mode."""
    return (getattr(settings, 'interface_mode', 'SIMPLE') == 'PRO'
            and bool(getattr(settings, 'contour_region_enabled', False)))


def active_region(scene, settings, depsgraph=None):
    if not region_enabled(settings):
        return None

...
class ContourTests(unittest.TestCase):
    def test_crossing_arcs_are_reordered_without_cutting_a_wall(self):
        points=[(0,0,1),(2,2,1),(0,2,1),(2,0,1)]
        clean=planner.uncross(points,lambda a,b:True)
        self.assertLess(planner.length(clean),planner.length(points))
        self.assertEqual(set(clean),set(points))
        self.assertEqual(planner.uncross(points,lambda a,b:False),points)

    def test_xy_crossing_on_different_floors_is_not_reordered(self):
        points=[(0,0,1),(2,2,1),(0,2,4),(2,0,4)]
        self.assertEqual(planner.uncross(points,lambda a,b:True),points)

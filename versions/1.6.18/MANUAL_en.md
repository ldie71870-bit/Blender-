...
## Optional CAD Plan Station Quotas

After path planning, enable `Station Quota CAD Plan` and click `Generate and Open CAD Plan`. The add-on temporarily renders a square orthographic Workbench image of the scene around the route bounds, restores all render settings, and opens the result in an Image Editor.

Blue lines show routes, cyan points show normal predicted stations, and magenta points show quota stations. Enter a station count, choose `Enter Count and Draw Rectangle`, then drag a green rectangle directly over the CAD image. The requested count is distributed by the length of all route fragments inside that rectangle. Regions cannot overlap.

The feature is disabled by default. When disabled, saved regions are ignored and the established sampling algorithm is unchanged. Existing minimum samples for detail routes remain a safety lower bound, so the panel may show an actual value above a requested value when a whole detail route lies inside a low quota region.

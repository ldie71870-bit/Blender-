...
def _scene_bounds(scene):
    ...
    for obj in scene.objects:
        if obj.type != "MESH" or region_volume.is_region(obj) or obj.get("gs_camera_mesh_visual"):
            continue
        try:
...
def _ray_cast(cache, origin, direction, distance):
    ...
    import sys
    hit, location, normal, face_index, obj, _matrix = sys.modules[__package__]._capture_ray_cast(
        cache.scene, cache.depsgraph, origin, direction, distance=distance
    )
...
def _heading_key_distances(points, cumulative, max_change_radians):
    ...
    for index in range(1, len(points) - 1):
        ...
        if before.length <= 1e-9 or after.length <= 1e-9:
            continue
        if before.angle(after) >= max_change_radians:
...
def _make_curve_candidate_groups(cache, components, component_counts, progress=None, cancel=None, cast_rays=True):
    ...
    for component_index, component in enumerate(components):
        route_object = component.get("object")
        explicit_height = bool(route_object is not None and route_object.get("gs_explicit_capture_height", False))
        layer_count = max(2, int(route_object.get("gs_capture_layer_count", 3))) if explicit_height else int(cache.settings.scientific_layer_count)
        layer_names = _layer_names(layer_count)
        base_layers = _base_layer_indices(layer_count)
        points = component["points"]
        cumulative = _polyline_metrics(points)
        ...
        for station_index, station in enumerate(stations):
            ...
            for slot, offset in enumerate(BASE_OFFSETS):
                distance = _clamp(station["distance"] + offset * station["step"], 0.0, cumulative[-1])
                point, tangent = _point_at(points, cumulative, distance)
                layer_index = min(layer_count-1, max(0, int(route_object.get("gs_capture_layer_index", 0)))) if explicit_height else base_layers[slot]
                layers = None if explicit_height else _layer_z_values(cache, point, layer_count)
                position = point.copy() if explicit_height else Vector((point.x, point.y, layers[layer_index]))
                ...
                if origin_key in used_origins:
                    ...
                    for attempt in range(1, 17):
                        ...
                        moved_layers = None if explicit_height else _layer_z_values(cache, moved_point, layer_count)
                        moved = moved_point.copy() if explicit_height else Vector((moved_point.x, moved_point.y, moved_layers[layer_index]))
                ...
                base = Candidate(
                    ...
                    pitch=math.radians(BASE_PITCHES[slot]),
                    kind="normal",
                    critical=critical,
                    is_base=True,
                )
                ...
                for yaw_index in range(yaw_count):
                    ...
                    for pitch_degrees in _allowed_pitches(layer_index, layer_count):
                        candidate = Candidate(
...
class CurveOriginProvider(OriginProvider):
    ...
    def generate_origins(self):
        ...
        for group in groups:
            ...
            route_object = component.get("object")
            effective_layer_count = max(2, int(route_object.get("gs_capture_layer_count", 3))) if (
                route_object is not None and route_object.get("gs_explicit_capture_height", False)
            ) else layer_count
            ...
            seeds.append(OriginSeed(
                ...
                polar_yaw=polar.yaw,
                allowed_pitches=_allowed_pitches(representative.layer_index, effective_layer_count),

...
assert len(st['cursor']['lines'])==96
for xyz in st['cursor']['lines']:
    p=Vector(xyz);assert abs(p.z-.004)<1e-5 and abs((Vector((p.x,p.y,0))-Vector((4,4,0))).length-radius)<1e-5
...
assert first>100 and st['batch'] is base
...
# Compare the previous implementation on the same BVH / KD tree and sample set.
old_pkg=types.ModuleType('old_brush_perf');old_pkg.__path__=[str(ROOT.parent/'_source_1.6.16')];sys.modules[old_pkg.__name__]=old_pkg
from old_brush_perf import coverage_heat as old
old_st=dict(st);old_st['selected']=set(st['selected']);old_st['batch']=base
old_times=[];old_calls=[];old_original=old.surface_audit.visible
old.surface_audit.visible=lambda *args:(old_calls.append(1),old_original(*args))[1]
for _ in range(80):
    t=time.perf_counter();old.paint(old_st,origin,direction,radius);old_times.append((time.perf_counter()-t)*1000)
assert old_st['batch'] is None and old_calls
...
assert len(curved['cursor']['lines'])==96
...
report=dict(samples=len(st['samples']),triangles=len(st['triangles']),total_chunks=len(st['selection_chunks']),local_dirty_chunks=len(st['selection_dirty']),
            first_dab_occlusion_rays=first_rays,repeated_old_occlusion_rays=len(old_calls),repeated_new_occlusion_rays=0,
            repeated_old_median_ms=statistics.median(old_times),repeated_new_median_ms=statistics.median(times),one_time_chunk_setup_s=prep,
            base_batch_preserved=True,curved_rim_projection=True,undo_keeps_cache=True)
out=ROOT.parent/'validation_1.6.17';out.mkdir(exist_ok=True);(out/'brush_performance.json').write_text(json.dumps(report,indent=2),encoding='utf8')

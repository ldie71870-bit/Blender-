...
def _path_inspection_geometry(scene, settings):
    ...
    if curve is None or curve.type != "CURVE" or scene.objects.get(curve.name) is None:
        return (), ()
    ...
    signature = (curve.as_pointer(), ray_m)
    if cached and cached[0] == signature:
        return cached[1], cached[2]
    cameras = path_inspection_cameras(scene, curve)
...
def _draw_path_camera_overlay():
    ...
    if not getattr(settings, "path_inspection_enabled", False):
        return
    ...
    for pitch_degrees in (-15.0, 0.0, 15.0):
        ...
        for index in range(azimuth_count):
            ...
            hit, point, normal, _face, obj, _matrix = scene.ray_cast(
                depsgraph, center + direction * 1e-4, direction, distance=search
            )
            if hit and obj and not is_camera_mesh_visual(obj) and not (
                obj.type == "CURVE" and (obj.get("gs_contour_version") or obj.get("gs_route_role"))
            ):
...
class GSCOLMAP_OT_refresh_path_inspection(Operator):
    ...
    def execute(self, context):
        settings = context.scene.gs_colmap_settings
        if not settings.path_inspection_enabled:
            self.report({"ERROR"}, "请先进入单路径检查模式")
            return {"CANCELLED"}
        invalidate_camera_overlay(context.scene)
        count = len(path_inspection_cameras(context.scene, settings.path_inspection_curve))
        tag_view3d_redraw()
...
def draw_gs_colmap_panel(self, context):
    ...
    try:
        ...
        if settings.rig_mode == "PATH":
            ...
            if settings.path_capture_mode == "LEGACY_PANORAMA_CUBE":
                ...
                if maximum_per_station > cameras_per_station:
                    budget.label(text=f"每站点相机数: {cameras_per_station} 基础 + 最多 {maximum_per_station - cameras_per_station} 补视角")
                else:
                ...
                budget.label(
                    text=(
                        f"预计总相机数: {station_count * cameras_per_station}–{station_count * maximum_per_station}"
                        if maximum_per_station > cameras_per_station
        ...
        inspect_box = box.box()
        inspect_box.label(text="单路径相机检查", icon="HIDE_OFF")
        if settings.path_inspection_enabled:

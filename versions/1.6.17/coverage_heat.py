...
def paint(st,origin,direction,radius,erase=False):
    ...
    for _,i,d in st['tree'].find_range(hit,radius):
        if (i in st['selected']) != erase:continue
        s=st['samples'][i]
        ...
        changed.add(i)
    selection_dirty(st,changed);return len(changed)
...
class GS_OT_coverage_brush(bpy.types.Operator):
    ...
    def invoke(self,context,event):
        ...
        self.st=st;self.navigating=False
    ...
    def modal(self,context,event):
        st=state(context.scene)
        if event.type in {'ESC','RET','NUMPAD_ENTER','WINDOW_DEACTIVATE'} or st is not self.st or not st or st['dirty'] or context.scene.gs_colmap_settings.coverage_busy:
        ...
        if event.type=='MIDDLEMOUSE':
            self.navigating=event.value!='RELEASE';st['cursor']=None;self.area.tag_redraw();return {'PASS_THROUGH'}
        if self.navigating:return {'PASS_THROUGH'}
        ...
        if event.type in {'LEFTMOUSE','MOUSEMOVE','INBETWEEN_MOUSEMOVE'}:
            ...
            if 0<=xy[0]<self.region.width and 0<=xy[1]<self.region.height:
                ...
                if self.dragging:paint(st,origin,direction,radius,event.shift)
                self.area.tag_redraw()
            else:

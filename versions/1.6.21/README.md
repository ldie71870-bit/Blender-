...
覆盖刷选改为真实 Mesh 表面的 24 段圆环与中心十字，移除空闲定时检测，并用 30 Hz / 2 像素门限、主命中复用和更轻的局部面片采样降低拖动卡顿。详见 `RELEASE_1.6.21_zh.md`。

当前版本：`1.6.9`

## 1.6.9 可编辑区域 Box 排线

“室内多层排线 → 轮廓排线”新增默认关闭的“启用区域排线”。在当前视图中心创建线框 Box 后，可以用 Blender 原生缩放、旋转和编辑模式加线、挤出，改成 L 形等封闭异形区域，然后点击“在区域内生成路径”。主线、细部短线、门洞探入和审计补线均受真实网格体积约束，凹角处的连接不能穿出区域。

区域辅助物不参与渲染、排线避障、球壳相机探测和导出点云采样。后台计算、进度和取消沿用轮廓排线流程。仅约束路径光心中心线，不裁剪相机视野。关闭区域功能后恢复原全场景或起点可达范围。

修复扩展加载器替换 `bl_info` 后界面退回旧版号的问题，N 面板显示 `GS UI v1.6.9`。

## 1.6.8 真实相机覆盖审计与直线探入

轮廓主线之外增加安全的直线探入短线，检查门洞、凹位与家具遮挡边缘。已有相机时使用实际视锥、裁剪面、首个网格命中和光心视差检查保留路径范围内的表面采样，仅保留能改善薄弱表面的补线。探入短线每站以 3–12 台相机定向观察最多四个目标，不再向无关方向铺满球壳。

N 面板“室内多层排线”中的“检查现有相机覆盖并补线”可直接作用于现有路径集合，保留原曲线。门洞补线和整体表面审计均有独立开关与数量参数；补线完成后重新生成相机。当前定向验证对应“旧全景立方体 → 球壳 12”模式。

“轻量相机代理”默认开启，普通视图和单路径检查均可显示青色光心与橙色朝向短线。代理不创建 Mesh 或场景对象，移动、删除相机后自动刷新，关闭时恢复原生相机显示尺寸。修复带厚度曲线误入路径和相机碰撞检测的问题。

审计为分物体抽样，不能等同于完整表面覆盖或高斯训练质量保证。

## 1.6.7 场景表面感知球壳补视角

球壳保留原有 12 方位，并可选依据当前 Blender 实际网格追加少量定向相机。每个站点以三档浅俯仰、24 个方位探测深度突变、物体切换和空间开口；低位遮挡边缘优先占用一半补拍预算，并以左右横移成对观察远侧表面，减少沙发与茶几、吧台凳脚和凹位被前景遮挡的问题。普通路径默认每站最多 4 台，细部短线最多再增加 2 台。

面板可关闭该功能，也可调每站补视角上限、探测距离和深度突变阈值。补充相机仍执行原有逐台净距检测，只保留安全位置。

## 1.6.6 可选 CAD 平面图站点配额

路径规划完成后，可选开启“站点配额平面图”。插件使用正交 Workbench 临时渲染生成灰白 CAD 风格户型底图，并在独立图像编辑器中叠加蓝色路径、青色常规站点和粉色配额站点。输入数量后直接在图上拖出绿色矩形，即可指定该区域沿路径放置的站点总数；多个不重叠区域可以叠加使用。

该功能默认关闭。关闭时不会读取区域配置，原有路径站点数量、细部短线最低采样和球壳生成逻辑保持不变。开启后先替换框内的常规采样，再按配额在框内曲线长度上均匀分配；细部短线既有最低站点数仍是安全下限。

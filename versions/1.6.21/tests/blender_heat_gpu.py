...
for area in bpy.context.screen.areas:
 if area.type=='VIEW_3D':
  area.spaces.active.region_3d.view_rotation=(Vector((0,0,1))-Vector((3,-6,3))).to_track_quat('-Z','Y')
  area.spaces.active.region_3d.view_location=(0,0,1);area.spaces.active.region_3d.view_distance=6
  area.spaces.active.overlay.show_overlays=False
errors=[];calls=[]
bpy.types.SpaceView3D.draw_handler_remove(h.HANDLE,'WINDOW')
def wrapped():
 try:h.draw();calls.append(1)
 except Exception as e:errors.append(repr(e))
h.HANDLE=bpy.types.SpaceView3D.draw_handler_add(wrapped,(),'WINDOW','POST_VIEW')
bpy.context.preferences.view.show_splash=False
h.redraw()
def done():
 import json
 out=root.parent/'validation_1.6.13';out.mkdir(exist_ok=True)
 (out/'gpu.json').write_text(json.dumps(dict(draws=len(calls),errors=errors,samples=len(h.state(scene)['samples']))),encoding='utf8')
 bpy.ops.screen.screenshot(filepath=str(out/'coverage.png'))
 bpy.ops.wm.quit_blender()
bpy.app.timers.register(done,first_interval=5.)

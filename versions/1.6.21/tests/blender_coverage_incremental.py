...
st=h.state(scene);base_sample_count=len(st['samples'])
...
assert not inc['selected'],'Successful incremental refresh must clear consumed brush selection'
assert inc['last_update']['mode']=='INCREMENTAL',inc['last_update']
counts=[len(v['views']) for v in samples];scores=[v['score'] for v in samples[:base_sample_count]]
for p,t in h.update_coverage(scene,s):pass
...
assert counts[:base_sample_count]==[len(v['views']) for v in full['samples']]

...
def select_lines(targets, observations, candidates, budget, baseline, angle, balanced=False,reserve_gaps=0):
    ...
    gap_counts={m:0 for m in counts};space_covered=set();pocket_counts={}
    while remaining and len(selected)<budget:
        ...
        gap_options=[v for v in options if gap_counts[remaining[v[1]].get('model',0)]<reserve_gaps
                     and remaining[v[1]].get('kind','GAP')=='GAP'
                     and len(remaining[v[1]].get('space_targets',set())-space_covered)>=5]
        if gap_options:
            def gap_score(option):
                c=remaining[option[1]]
                repeats=pocket_counts.get((c.get('model',0),c.get('pocket',option[1])),0)
                return len(c.get('space_targets',set())-space_covered)/((.5+.3*c['length'])*(1+.35*repeats))
            options=[(gap_score(v),v[1],v[2]) for v in gap_options]
        ...
        if new_space and 'pocket' in candidate:
            key=(candidate.get('model',0),candidate['pocket']);pocket_counts[key]=pocket_counts.get(key,0)+1

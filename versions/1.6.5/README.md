...
选中一条路径后，通过 N 面板按钮或 `Alt + Shift + I` 进入检查模式：完整场景保留，仅隐藏其他路径和插件生成相机的原生大线框。当前路径上的相机用青色 GPU 点与橙色短朝向线批量绘制，可调点大小、米制朝向长度与穿透显示。代理不创建任何对象、Mesh 或材质，并缓存相机数据和 GPU 批次以保持视口流畅；退出时恢复原显示状态。绿色金字塔代理功能已移除，旧版残留会自动清理。

## 1.6.4 球壳相机站点预算修复

轮廓排线包含大量细部线时，球壳相机生成会把每条细部线的最低采样站点计入真实预算。旧的 120 站点设置不足时自动扩展，不再中止并留下空相机集合；面板同时显示扩展后的站点数和预计相机总数。每个站点的 12 台相机独立检查，只剔除贴近或穿过地面、墙面及物体的单台相机，其余相机保留。相邻站点默认绕竖直轴递增旋转 5°，旋转步长和表面净距都可在面板调节。
...
- 在 3D 视图选中一条路径，点击 N 面板“孤立所选路径并查看相机”，或按 `Alt + Shift + I`。
- 完整场景保持可见，仅隐藏同一路径集合中的其他曲线和生成相机的原生大线框。
- 相机位置显示为青色 GPU 点，镜头方向显示为橙色短线；可调整点大小、朝向线长度和穿透显示。
- 代理使用一个视口绘制回调，不创建对象、Mesh 或材质。再次点击按钮或按快捷键会恢复进入前的显示状态。
- 绿色金字塔 Mesh 代理功能已经移除；加载文件或生成新阵列时会清理旧版残留代理。

...
def refine(main_routes, models, probe, settings, progress):
    ...
    def discover(index,direction,distance):
        ...
        key=(obj.original.name,*(round(v/precision) for v in point),*(round(v*3) for v in normal))
        target_index=target_keys.get(key)
        if target_index is None:
            target_index=len(targets);target_keys[key]=target_index
            targets.append(dict(point=tuple(point),normal=tuple(normal),object=obj.original.name,
    ...
    for index,target in enumerate(targets):
        views=[]
        for _,origin_index,distance in main_tree.find_n(target['point'],min(16,len(sample_points))):
            if distance>max_distance:break
            point=sample_points[origin_index]
    ...
    visible_cache.clear()
    ranked=sorted(range(len(candidates)),key=lambda i:sum(targets[t]['weight'] for t in candidates[i]['targets']-already),reverse=True)
    line_candidates=[]
    center_keys=set()
    ...
    for index in ranked:
        ...
        center_key=tuple(round(v/.35) for v in center)
        ...
        tangent.normalize()
        best=None
        for direction in (tangent,Vector((-tangent.y,tangent.x,0))):
            for half_length in (.60,.45,.30):
    ...
    selected,before,after=metrics.select_lines(targets,observations,line_candidates,
                                               int(settings.contour_detail_budget),baseline,angle)
    result=[]
    details=[]
    for candidate in selected:
        ...
        details.append(dict(layer=model['label'],new_surface_cells=candidate['gain'],
                            length_m=candidate['length'],target_objects=route.target_objects))
    report=dict(target_count=len(targets),main_observed_cells=len(before),final_observed_cells=len(after),
                ...
                maximum_observation_distance_m=max_distance,scope='sampled_candidate_observable_surfaces',

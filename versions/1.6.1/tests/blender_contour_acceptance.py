...
objects=list(settings.path_collection.objects)
...
assert min(p.co.z for o in settings.path_collection.objects for p in o.data.splines[0].points)>3

# Scientific camera sampling must use the explicit curve Z, not infer new layers.
from mathutils import Vector
science=addon.scientific_planner
from types import SimpleNamespace
obj=next(iter(settings.path_collection.objects))

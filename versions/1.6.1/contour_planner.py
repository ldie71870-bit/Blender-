...
def join_routes(routes, cells, graph, origin, step, valid, max_bridge):
    ...
    while work:
        main = work.pop(0)
        while work:
            # Every main-route vertex can host an excursion, even for open routes.
            root = route_anchors(Route(main.points, closed=True), cells, by_xy, origin, step, valid)
            targets = {}
            ...
            if other.closed:
                body = other.points[:-1]
                excursion = body[index:]+body[:index+1]
            else:
                body = other.points if index == 0 else list(reversed(other.points))
                excursion = body+body[-2::-1]
            joined = bridge+excursion[1:]+list(reversed(bridge[:-1]))
            main.points[main_index:main_index+1] = joined
            main.kind = 'CONTOUR_NETWORK'
            bridge_length += length(bridge)*2
        result.append(main)

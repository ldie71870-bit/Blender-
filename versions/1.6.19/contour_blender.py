...
from . import doorway_peek
from . import cross_grid
from . import region_volume


class SceneProbe:
    def __init__(self, scene, scale, margin, step, geometry=None, region=None):
        self.scene = scene
    ...
    def build_geometry(self):
        ...
        for instance in self.depsgraph.object_instances:
            obj=instance.object
            if obj.type!='MESH' or region_volume.is_region(obj) or obj.get('gs_camera_mesh_visual') or obj.get('gs_contour_version') or obj.original.get('gs_contour_version'):continue
    ...
    def cast(self, point, direction, distance):
        ...
        for _ in range(64):
            ...
            position*=self.scale
            if not obj.get('gs_contour_version') and not region_volume.is_region(obj):return tuple(position),normal,obj
            remaining-=(position-start).length+.00001
            if remaining<=0:return None
            start=position+direction*.00001
        raise ValueError('射线穿过过多辅助物，请检查重叠的预览曲线和区域框')
    ...
    def point_clear(self, point):
        if self.region and not self.region.contains(point):return False
    ...
    def segment_clear(self, left, right):
        ...
        if self.region and not self.region.segment_inside(left,right):
            self.segment_cache[key]=False
            return False
...
def geometry_bounds(scene, probe):
    ...
    for instance in probe.depsgraph.object_instances:
        obj = instance.object
        if obj.type != 'MESH' or region_volume.is_region(obj) or obj.get('gs_contour_version') or obj.get('gs_camera_mesh_visual'):
            continue
...
def make_layer_graph(supports, height, probe, step, max_step, progress, ceiling_gap=None, minimum_height=.5, follow_ceiling=False):
    values = []
    expected_by_xy=defaultdict(list)
    for index, (key, floor) in enumerate(supports.items()):
        target_z=floor[2]+height
        if ceiling_gap is not None and (follow_ceiling or not probe.point_clear((floor[0],floor[1],target_z))):
            ...
            ceiling_z=overhead[0][2]-max(ceiling_gap,probe.margin+.03)
            if follow_ceiling and ceiling_z < target_z+.65:continue
            target_z=ceiling_z if follow_ceiling else min(target_z,ceiling_z)
...
def build_models(supports, specs, probe, step, settings, seed, progress):
    ...
    for index,(label,height) in enumerate(specs):
        ...
        ceiling_gap=settings.floorplan_ceiling_offset if (label=='Ceiling' or (label=='High' and settings.contour_adapt_top)) else None
        minimum_height=(settings.floorplan_mid_height+.15 if len(specs)>=3 else settings.floorplan_low_height+.15)
        cells,graph,valid=make_layer_graph(supports,height,probe,step,settings.contour_max_step,progress,ceiling_gap,minimum_height,follow_ceiling=label=='Ceiling')
        models.append(dict(index=index,label=label,height=height,cells=cells,graph=graph,valid=valid,step=step))
    if settings.floorplan_space_mode=='ALL' or probe.region:
...
def generate(scene, settings, progress=None):
    ...
    previous=[(obj,obj.hide_get()) for obj in scene.objects
              if region_volume.is_region(obj) or (obj.type=='CURVE' and obj.get('gs_contour_version'))]
    result=None
    try:
...
class PlanningSettings:
    """Per-run overrides; keep hidden professional preferences intact."""
    def __init__(self, settings, **overrides):
        self.source=settings
        self.overrides=overrides
    def __getattr__(self, name):
        return self.overrides[name] if name in self.overrides else getattr(self.source,name)


def effective_planning_settings(settings):
    if getattr(settings,'interface_mode','PRO')!='SIMPLE':return settings
    return PlanningSettings(settings,contour_detail_enabled=True,
        contour_peek_enabled=True,contour_adapt_top=True,contour_ceiling_detail_enabled=True,
        contour_detail_budget=max(48,settings.contour_detail_budget),
        contour_detail_grid=min(.15,settings.contour_detail_grid),
        contour_detail_clearance=min(.10,settings.contour_detail_clearance))


def _generate(scene, settings, progress=None):
    settings=effective_planning_settings(settings)
    ...
    probe = SceneProbe(scene, scale, margin, step,region=region)
    bounds = geometry_bounds(scene, probe)
    probe.vertical_range = bounds[1].z-bounds[0].z+2
    if region:bounds=region.sampling_bounds(bounds)
    ...
    for model in layer_models:
        ...
        if getattr(settings,'contour_cross_grid_enabled',True):
            progress('生成交叉井字骨架',layer_index,len(layer_models))
            cross,cross_report=cross_grid.lanes(cells,graph,layer_valid,step,settings.contour_cross_grid_spacing,settings.contour_cross_grid_budget)
            routes.extend(cross);stats['cross_grid']=cross_report
    ...
    if settings.contour_detail_enabled:
        ...
        detail_probe=SceneProbe(scene,scale,detail_margin,detail_step,geometry=probe.geometry,region=region)
        ...
        detail_specs=list(specs)
        if settings.contour_ceiling_detail_enabled and mode!='ONE':
            detail_specs.append(('Ceiling',float(settings.floorplan_top_height)))
        detail_settings=PlanningSettings(settings,contour_min_area=min(.08,settings.contour_min_area))
        detail_models=build_models(detail_supports,detail_specs,detail_probe,detail_step,detail_settings,seed,progress)
    ...
    peek_report = None
    peek_routes = []
    if getattr(settings, 'contour_peek_enabled', True):
        peek_routes, peek_report = doorway_peek.discover(all_routes, probe, settings.contour_peek_budget, settings.contour_peek_depth, progress)
        all_routes.extend(peek_routes)
    camera_audit = None
    if getattr(settings,'contour_camera_audit_enabled',False):
        import sys
        from . import surface_audit
        addon=sys.modules[__package__]
        existing=addon._generated_path_cameras(scene,settings)
        if existing:
            extra,camera_audit=surface_audit.audit_and_refine(scene,all_routes,probe,existing,settings.contour_audit_budget,progress)
            checked,verification=surface_audit.validate_routes(scene,settings,probe,peek_routes+extra,camera_audit,progress)
            if peek_routes:all_routes=all_routes[:-len(peek_routes)]
            all_routes.extend(checked);camera_audit['actual_camera_verification']=verification
    report = dict(version='1.6.12',region=region.name if region else None,method='LAYERED_CONTOUR',columns=columns,support_samples=len(supports),
                  ...
                  main_route_count=main_route_count, detail_coverage=detail_report, doorway_peek=peek_report, camera_surface_audit=camera_audit,
    ...
    if region and any(not region.segment_inside(a,b) for _,_,r in all_routes for a,b in zip(r.points,r.points[1:])):
        raise ValueError('路径越出区域边界，已取消本次生成')
    collection = bpy.data.collections.new('GS_Region_Preview' if region else 'GS_Contour_Preview')
    ...
    try:
        ...
        groups={}
        materials={}
        labels={'Low':'低层','Middle':'中层','High':'顶层','Middle_2':'高中层','Ceiling':'挑空顶层'}
        for index,(layer_index,label,route) in enumerate(all_routes):
            ...
            name = f'GS_{"Grid" if route.kind == "CROSS_GRID" else "Detail" if route.kind == "DETAIL" else "Contour"}_{label}_{index+1:03d}'
            ...
            if region:obj['gs_path_region_source']=region.name
            ...
            obj['gs_capture_layer_count'] = max(3,max(i for i,_,_ in all_routes)+1)
            ...
            if route.kind == 'DETAIL':
                ...
                if getattr(route, 'peek_route', False):
                    obj['gs_peek_route'] = True
                    obj['gs_peek_target'] = tuple(v/scale for v in route.peek_target)

...
def surface_hit(st,origin,direction):
    """Return one reusable visible-surface hit for cursor drawing and painting."""
    return st['probe'].geometry[0].ray_cast(origin,direction)


def mouse_sample_due(last_xy,xy,last_time,now,force=False):
    """Reject idle/jitter events before constructing view rays or touching GPU data."""
    if force or last_xy is None:return True
    dx=xy[0]-last_xy[0];dy=xy[1]-last_xy[1]
    return dx*dx+dy*dy>=CURSOR_PIXEL_STEP*CURSOR_PIXEL_STEP and now-last_time>=CURSOR_INTERVAL


def surface_cursor(st,origin,direction,radius,erase=False,area=None,hit_info=None):
    """Project the brush rim onto the hit surface; break at silhouettes/edges."""
    bvh=st['probe'].geometry[0]
    hit,n,face,dist=hit_info if hit_info is not None else surface_hit(st,origin,direction)
    ...
    for i in range(CURSOR_SEGMENTS):
        point=hit+radius*(u*math.cos(i*math.tau/CURSOR_SEGMENTS)+v*math.sin(i*math.tau/CURSOR_SEGMENTS))
    ...
    # A small surface-projected cross makes the active hit unambiguous without
    # creating a mesh proxy or relying on Blender's paint-brush cursor icon.
    center=(hit+n*.004)/st['scale']
    for tangent in (u,v):
        ends=[]
        for sign in (-1,1):
            point=hit+tangent*(radius*.14*sign)
            p,normal,_,_=bvh.ray_cast(point+n*radius*.3,-n,radius*.6)
            ends.append((p+normal*.004)/st['scale'] if p is not None and normal.dot(n)>.3 else None)
        if all(p is not None for p in ends):lines.extend((tuple(ends[0]),tuple(ends[1])))
        elif ends[0] is not None:lines.extend((tuple(ends[0]),tuple(center)))
        elif ends[1] is not None:lines.extend((tuple(center),tuple(ends[1])))
...
def paint(st,origin,direction,radius,erase=False,hit_info=None):
    return surface_brush.paint(st,origin,direction,radius,erase,hit_info)
...
class GS_OT_coverage_brush(bpy.types.Operator):
    ...
    def invoke(self,context,event):
        ...
        context.window_manager.modal_handler_add(self);context.window.cursor_modal_set('CROSSHAIR');self.hover_time=0.;self.hover_xy=None
    ...
    def modal(self,context,event):
        ...
        if event.type in {'LEFTMOUSE','MOUSEMOVE','INBETWEEN_MOUSEMOVE'}:
            ...
            if 0<=xy[0]<self.region.width and 0<=xy[1]<self.region.height:
                now=time.monotonic();force=event.type=='LEFTMOUSE'
                if not mouse_sample_due(self.hover_xy,xy,self.hover_time,now,force):return {'RUNNING_MODAL'}
                ...
                hit_info=surface_hit(st,origin,direction)
                surface_cursor(st,origin,direction,radius,event.shift,self.area.as_pointer(),hit_info)
                self.hover_time=now;self.hover_xy=xy
                if self.dragging:paint(st,origin,direction,radius,event.shift,hit_info)
                self.area.tag_redraw()
            else:

...
assert st['selected'] and all((st['brush_patches'][i] if i<0 else st['samples'][i])['owner']=='Front' for i in st['selected'])
...
for p,t in h.analysis(scene,s):pass
new=h.state(scene);assert new['samples'] and new['records']
class Layout:
...
import json
out=root.parent/'validation_1.6.21';out.mkdir(exist_ok=True)
(out/'checks.json').write_text(json.dumps(dict(samples=len(st['samples']),added=len(added),occlusion=True,brush_front_only=True,duplicate_centres_not_counted=True,both_modes=True,coverage_recomputed=True)),encoding='utf8')

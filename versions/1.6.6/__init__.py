    ...
    if (
        ...
        if array_mode == "SPHERICAL_SHELL_12":
            for key in (
                ...
                "shell_station_rotation_step", "shell_failure_policy"
            ):
...
def _generate_spherical_shell_12_group(
    ...
    directions, frame = _world_shell_directions(path_forward, phase)
            ...
            if shell_path:
                ...
                for index, (point, tangent, path_object) in enumerate(sampled):
                    ...
                    if station_radii:
                        ...
                        filtered_camera_count += max(0, 12 - len(station_radii))
...
class GSCOLMAP_OT_create_cameras(Operator):
    ...
    def _record_station_result(
        self, context, group, actual_radii, used_fallback, skipped, debug_spec
    ):
        ...
        if actual_radii:
            ...
            self._filtered_camera_count += max(0, 12 - len(actual_radii))
    ...
    def _finish_unsafe_station(self, context, state, fallback_safe):
        ...
        if use_fallback and fallback_safe:
            ...
            self._record_station_result(context, group, [], True, False, None)
        else:
            self._record_station_result(context, [], [], False, True, None)

from . import addon_lifecycle
addon_lifecycle.before_reload(globals())
...
# Blender's extension loader may replace bl_info after importing the module.
_ADDON_VERSION = ".".join(str(value) for value in bl_info["version"])
...
from . import capture_collision
from .contour_progress import read as read_contour_progress
from . import region_volume


def _legacy_contour_status(job):
    """Keep a modal class from a pre-1.6.22 reload usable."""
    return read_contour_progress(job)[0]


def _legacy_contour_fraction(job):
    """Keep a modal class from a pre-1.6.22 reload usable."""
    return read_contour_progress(job)[1]


# Blender can retain a submodule and an operator class across an extension
# update. Old path-generation modals call contour_jobs.status/fraction, while
# newer modules read the progress side channel directly. Restore only missing
# APIs so either generation can finish the worker it already started.
if not callable(getattr(contour_jobs, "status", None)):
    contour_jobs.status = _legacy_contour_status
if not callable(getattr(contour_jobs, "fraction", None)):
    contour_jobs.fraction = _legacy_contour_fraction
...
I18N = {
    "en": {
        ...
        "export_only": "Export Sparse Point Cloud Only",
        "rig": "Rig",
...
SHELL_RADIUS_MODE_ITEMS = (
    ...
    ("CLEARANCE_ADAPTIVE", "净空自适应", "每个方向独立缩小半径；无法避开表面时只剔除该台相机。"),
)

SHELL_FAILURE_POLICY_ITEMS = (
    ("LEGACY_SIX_FALLBACK", "回退旧六相机", "一个站点的12个方向全部无效时，在安全站点中心生成旧六相机。"),
    ("SKIP_STATION", "跳过站点", "一个站点的12个方向全部无效时跳过该站点。"),
)
...
_RENDER_SIG_KEYS = (
    ...
    "render_rgb", "export_depth", "depth_format", "export_object_depth",
...
def render_signature(settings, total):
    ...
    if (
        ...
        if array_mode == "SPHERICAL_SHELL_12":
            for key in (
                "shell_radius", "shell_radius_mode", "shell_min_radius", "shell_surface_clearance",
                "shell_station_rotation_step", "shell_failure_policy", "shell_surface_detail_enabled",
                "shell_surface_detail_cameras", "shell_surface_probe_distance", "shell_surface_depth_jump",
            ):
                sig[key] = getattr(settings, key, None)
    sig["scene_exposure"] = float(getattr(getattr(getattr(settings, "id_data", None), "view_settings", None), "exposure", 0.0))
    sig["total"] = int(total)
...
_CAMERA_OVERLAY_HANDLE = None
_STATION_PLAN_IMAGE_HANDLE = None
_CAMERA_OVERLAY_CACHE = {}
_CAMERA_OVERLAY_GPU_CACHE = {}
_PATH_INSPECTION_STATES = {}
_PATH_INSPECTION_KEYMAPS = []
_STATION_PLAN_VIEW_STATES = {}
_STATION_PLAN_PREVIEW = None
STATION_PLAN_IMAGE_NAME = "GS_Station_CAD_Plan"
...
def sync_camera_mesh_visuals(scene, settings, cameras):
    """Remove legacy pyramid proxies; 1.6.5 uses a zero-object GPU overlay."""
    ...
    for obj in tuple(scene.objects):
        if is_camera_mesh_visual(obj):
            bpy.data.objects.remove(obj, do_unlink=True)
    invalidate_camera_overlay(scene)
    sync_native_camera_sizes(scene,getattr(settings,"camera_overlay_enabled",False))
    return 0
...
def invalidate_camera_overlay(scene=None):
    if scene is None:
        _CAMERA_OVERLAY_CACHE.clear()
        _CAMERA_OVERLAY_GPU_CACHE.clear()
    else:
        _CAMERA_OVERLAY_CACHE.pop(scene.as_pointer(), None)
        _CAMERA_OVERLAY_GPU_CACHE.pop(scene.as_pointer(), None)


@persistent
def cleanup_legacy_camera_mesh_visuals(_unused=None):
    """Delete all generated pyramid objects and their now-retired data blocks."""
    for camera in tuple(obj for obj in bpy.data.objects if obj.type == "CAMERA"):
        remove_camera_mesh_visual(camera)
    for obj in tuple(bpy.data.objects):
        if is_camera_mesh_visual(obj):
            bpy.data.objects.remove(obj, do_unlink=True)
    collection = bpy.data.collections.get(CAMERA_VISUAL_COLLECTION)
...
@persistent
def restore_transient_view_modes(_unused=None):
    """Do not persist temporary isolation, top-view, or in-front display state."""
    restore_inspection = globals().get("_restore_path_inspection")
    restore_plan = globals().get("_restore_station_plan_view")
    for scene in tuple(bpy.data.scenes):
        if restore_inspection is not None:
            restore_inspection(scene)
        if restore_plan is not None:
            restore_plan(scene)


def path_inspection_cameras(scene, curve):
    if scene is None or curve is None:
        return []
    curve_name = curve.name
    return sorted(
        (obj for obj in scene.objects
         if obj.type == "CAMERA" and obj.get("source_curve", "") == curve_name),
        key=lambda obj: (int(obj.get("station_index", -1)),
                         int(obj.get("shell_camera_index", -1)), obj.name),
    )


def _path_inspection_geometry(scene, settings):
    curve = getattr(settings, "path_inspection_curve", None)
    isolated = bool(getattr(settings, "path_inspection_enabled", False))
    if isolated and (curve is None or curve.type != "CURVE" or scene.objects.get(curve.name) is None):
        return (), ()
    cache_key = scene.as_pointer()
    ray_m = max(0.005, float(getattr(settings, "path_inspection_ray_length", 0.12)))
    cached = _CAMERA_OVERLAY_CACHE.get(cache_key)
    signature = (curve.as_pointer() if isolated else 0, ray_m)
    if cached and cached[0] == signature:
        return cached[1], cached[2]
    cameras = path_inspection_cameras(scene, curve) if isolated else _generated_path_cameras(scene, settings)
    scale_length = float(getattr(scene.unit_settings, "scale_length", 1.0) or 1.0)
    ray_length = ray_m / max(1e-9, scale_length)
    points = []
    lines = []
    for camera in cameras:
        ...
        if forward.length_squared <= 1e-12:
            continue
        forward.normalize()
        points.append(tuple(origin))
        lines.extend((tuple(origin), tuple(origin + forward * ray_length)))
    result = (tuple(points), tuple(lines))
    _CAMERA_OVERLAY_CACHE[cache_key] = (signature, result[0], result[1])
    return result


def _draw_path_camera_overlay():
    if gpu is None or batch_for_shader is None:
        return
    context = bpy.context
    scene = getattr(context, "scene", None)
    if scene is None or not hasattr(scene, "gs_colmap_settings"):
        return
    settings = scene.gs_colmap_settings
    if not (settings.path_inspection_enabled or settings.camera_overlay_enabled):
        return
    points, lines = _path_inspection_geometry(scene, settings)
    if not points:
        return
    shader = gpu.shader.from_builtin("UNIFORM_COLOR")
    cache_key = scene.as_pointer()
    signature = _CAMERA_OVERLAY_CACHE[cache_key][0]
    gpu_cached = _CAMERA_OVERLAY_GPU_CACHE.get(cache_key)
    if not gpu_cached or gpu_cached[0] != signature:
        gpu_cached = (
            signature,
            batch_for_shader(shader, "POINTS", {"pos": points}),
            batch_for_shader(shader, "LINES", {"pos": lines}),
        )
        _CAMERA_OVERLAY_GPU_CACHE[cache_key] = gpu_cached
    point_batch, line_batch = gpu_cached[1], gpu_cached[2]
    try:
        gpu.state.blend_set("ALPHA")
        gpu.state.depth_test_set("NONE" if settings.path_inspection_xray else "LESS_EQUAL")
        gpu.state.line_width_set(1.5)
        shader.bind()
        shader.uniform_float("color", (1.0, 0.48, 0.08, 0.92))
        line_batch.draw(shader)
        gpu.state.point_size_set(float(settings.path_inspection_marker_size))
        shader.uniform_float("color", (0.02, 0.88, 1.0, 1.0))
...
@persistent
def camera_overlay_scene_changed(scene, depsgraph):
    if any(update.is_updated_transform or update.is_updated_geometry or
           isinstance(update.id,(bpy.types.Scene,bpy.types.Collection,bpy.types.Object,bpy.types.Camera))
           for update in depsgraph.updates):
        invalidate_camera_overlay(scene)
        tag_view3d_redraw()


def draw_path_camera_overlay():
    _draw_path_camera_overlay()


def _station_plan_bounds(settings):
    points = [point for item in path_components(settings) for point in item["points"]]
    if not points:
        raise RuntimeError("没有可用于生成平面图的路径曲线")
    ...
    size = span + margin * 2.0
    return (center_x - size * 0.5, center_x + size * 0.5,
            center_y - size * 0.5, center_y + size * 0.5,
            max(point.z for point in points))


def render_station_cad_plan(context):
    scene = context.scene
    settings = scene.gs_colmap_settings
    x_min, x_max, y_min, y_max, top_path_z = _station_plan_bounds(settings)
    resolution = int(settings.station_plan_resolution)
    center_x = (x_min + x_max) * 0.5
    center_y = (y_min + y_max) * 0.5
    ortho_scale = max(x_max - x_min, y_max - y_min)
    camera_data = bpy.data.cameras.new("GS Station CAD Camera Data")
    camera = bpy.data.objects.new("GS Station CAD Camera", camera_data)
    scene.collection.objects.link(camera)
    units_per_meter = 1.0 / max(1e-9, float(scene.unit_settings.scale_length or 1.0))
    camera.location = (center_x, center_y, top_path_z + 0.05 * units_per_meter)
    camera.rotation_euler = (0.0, 0.0, 0.0)
    camera_data.type = "ORTHO"
    camera_data.ortho_scale = ortho_scale
    camera_data.clip_start = 0.01
    camera_data.clip_end = max(1000.0, ortho_scale * 10.0)
    ...
    old_world_color = scene.world.color[:] if scene.world is not None else None
    shading_names = (
        "light", "color_type", "single_color", "show_shadows", "show_cavity",
        "cavity_type", "show_outline", "object_outline_color", "background_type", "background_color",
    )
    old_shading = {name: getattr(shading, name) for name in shading_names if hasattr(shading, name)}
    hidden = []
    plan_path = os.path.join(tempfile.gettempdir(), "gs_station_cad_plan.png")
    for obj in tuple(scene.objects):
        if obj == camera:
            continue
        if obj.type in {"CAMERA", "CURVE", "EMPTY", "LIGHT"} or obj.get("gs_shell_debug_proxy"):
            hidden.append((obj, bool(obj.hide_render)))
    ...
    try:
        ...
        if hasattr(shading, "single_color"):
            shading.single_color = (0.68, 0.72, 0.76)
        ...
        if hasattr(shading, "object_outline_color"):
            shading.object_outline_color = (0.03, 0.04, 0.05)
        if hasattr(shading, "background_type"):
            shading.background_type = "WORLD"
        if hasattr(shading, "background_color"):
            shading.background_color = (0.96, 0.96, 0.96)
        if scene.world is not None:
            scene.world.color = (1.0, 1.0, 1.0)
        bpy.ops.render.render(write_still=True)
        if not os.path.isfile(plan_path):
            raise RuntimeError("CAD 平面底图渲染失败")
        old_image = bpy.data.images.get(STATION_PLAN_IMAGE_NAME)
        if old_image is not None:
            bpy.data.images.remove(old_image)
        image = bpy.data.images.load(plan_path, check_existing=False)
        image.name = STATION_PLAN_IMAGE_NAME
        try:
            image.pack()
        except RuntimeError:
            pass
        ...
        scene.camera = old_render["camera"]
        render.engine = old_render["engine"]
        render.resolution_x = old_render["resolution_x"]
        render.resolution_y = old_render["resolution_y"]
        render.resolution_percentage = old_render["resolution_percentage"]
        render.film_transparent = old_render["film_transparent"]
        render.filepath = old_render["filepath"]
        render.image_settings.file_format = old_render["file_format"]
        render.image_settings.color_mode = old_render["color_mode"]
        for name, value in old_shading.items():
        ...
        try:
            ...
            pass
    settings.station_plan_x_min = x_min
    settings.station_plan_x_max = x_max
    settings.station_plan_y_min = y_min
    settings.station_plan_y_max = y_max
    invalidate_station_plan(scene)
    return image


def _station_image_world_to_region(context, settings, point):
    width = max(1e-9, settings.station_plan_x_max - settings.station_plan_x_min)
    height = max(1e-9, settings.station_plan_y_max - settings.station_plan_y_min)
    u = (point[0] - settings.station_plan_x_min) / width
    v = (point[1] - settings.station_plan_y_min) / height
    x, y = context.region.view2d.view_to_region(u, v, clip=False)
    return (float(x), float(y), 0.0)


def _station_image_mouse_world(context, event, settings, region=None):
    ...
    v = min(1.0, max(0.0, v))
    return Vector((
        settings.station_plan_x_min + u * (settings.station_plan_x_max - settings.station_plan_x_min),
        settings.station_plan_y_min + v * (settings.station_plan_y_max - settings.station_plan_y_min),
        0.0,
    ))


def draw_station_plan_image_overlay():
    if gpu is None or batch_for_shader is None:
        return
    context = bpy.context
    scene = getattr(context, "scene", None)
    space = getattr(context, "space_data", None)
    if scene is None or not hasattr(scene, "gs_colmap_settings") or space is None:
        return
    settings = scene.gs_colmap_settings
    image = getattr(space, "image", None)
    if (not settings.station_plan_enabled or not settings.station_plan_view_active
            or image is None or image.name != STATION_PLAN_IMAGE_NAME):
        return
    regions = active_station_regions(settings)
    samples = sample_path_components(settings)
    shader = gpu.shader.from_builtin("UNIFORM_COLOR")
    try:
        gpu.state.blend_set("ALPHA")
        gpu.state.depth_test_set("NONE")
        ...
        for component in path_components(settings):
            ...
            for index in range(1, len(points)):
                path_vertices.extend((
                    _station_image_world_to_region(context, settings, points[index - 1]),
                    _station_image_world_to_region(context, settings, points[index]),
                ))
        if path_vertices:
            gpu.state.line_width_set(2.0)
            shader.uniform_float("color", (0.02, 0.45, 1.0, 0.92))
            batch_for_shader(shader, "LINES", {"pos": path_vertices}).draw(shader)
        outside = []
        inside = []
        for point, _tangent, _obj in samples:
            target = inside if any(point_in_station_region(point, region) for region in regions) else outside
            target.append(_station_image_world_to_region(context, settings, point))
        gpu.state.point_size_set(float(settings.station_plan_marker_size))
        if outside:
            shader.uniform_float("color", (0.0, 0.85, 1.0, 1.0))
            batch_for_shader(shader, "POINTS", {"pos": outside}).draw(shader)
        if inside:
            shader.uniform_float("color", (1.0, 0.08, 0.48, 1.0))
            batch_for_shader(shader, "POINTS", {"pos": inside}).draw(shader)
        gpu.state.line_width_set(2.5)
        for index, region in enumerate(regions):
            x0, x1, y0, y1 = station_region_bounds(region)
            corners = [
                _station_image_world_to_region(context, settings, (x0, y0)),
                _station_image_world_to_region(context, settings, (x1, y0)),
                _station_image_world_to_region(context, settings, (x1, y1)),
                _station_image_world_to_region(context, settings, (x0, y1)),
            ]
            vertices = (corners[0], corners[1], corners[1], corners[2],
                        corners[2], corners[3], corners[3], corners[0])
            shader.uniform_float("color", (0.15, 0.95, 0.16, 1.0))
            batch_for_shader(shader, "LINES", {"pos": vertices}).draw(shader)
            blf.color(0, 0.05, 0.20, 0.05, 1.0)
            blf.size(0, 14.0)
            blf.position(0, corners[3][0] + 5.0, corners[3][1] - 18.0, 0.0)
            blf.draw(0, f"区域 {index + 1}: {region.station_count} 站")
        if _STATION_PLAN_PREVIEW:
            x0, y0, x1, y1 = _STATION_PLAN_PREVIEW
            corners = [
                _station_image_world_to_region(context, settings, (x0, y0)),
                _station_image_world_to_region(context, settings, (x1, y0)),
                _station_image_world_to_region(context, settings, (x1, y1)),
            ...
            vertices = (corners[0], corners[1], corners[1], corners[2],
                        corners[2], corners[3], corners[3], corners[0])
            shader.uniform_float("color", (1.0, 0.72, 0.0, 1.0))
            batch_for_shader(shader, "LINES", {"pos": vertices}).draw(shader)
    finally:
        gpu.state.line_width_set(1.0)
        gpu.state.point_size_set(1.0)
...
def sync_native_camera_sizes(scene, enabled):
    for camera in _generated_path_cameras(scene, scene.gs_colmap_settings):
        data=camera.data
        key="gs_overlay_original_size"
        if enabled:
            if key not in data:data[key]=float(data.display_size)
            if abs(data.display_size-.01)>1e-6:data.display_size=.01
        elif key in data:
            data.display_size=float(data[key]);del data[key]


@persistent
def sync_camera_overlay_on_load(_unused=None):
    invalidate_camera_overlay()
    for scene in bpy.data.scenes:
        if hasattr(scene,"gs_colmap_settings"):
            sync_native_camera_sizes(scene,scene.gs_colmap_settings.camera_overlay_enabled)
    tag_view3d_redraw()


def update_path_inspection_overlay(self, context):
    scene = getattr(context, "scene", None) if context else None
    invalidate_camera_overlay(scene)
    if scene and hasattr(scene,"gs_colmap_settings"):
        sync_native_camera_sizes(scene,scene.gs_colmap_settings.camera_overlay_enabled)
    tag_view3d_redraw()


def invalidate_station_plan(scene=None):
    return None


def update_station_plan(self, context):
    scene = getattr(context, "scene", None) if context else None
    if scene is not None and hasattr(self, "station_plan_enabled"):
        if not self.station_plan_enabled and getattr(self, "station_plan_view_active", False):
            restore = globals().get("_restore_station_plan_view")
...
class GSCOLMAP_StationRegion(PropertyGroup):
    name: StringProperty(name="区域名称", default="重点区域")
    enabled: BoolProperty(name="启用", default=True, update=update_station_plan)
    station_count: IntProperty(
        name="目标站点数", default=24, min=1, max=10000,
        description="该矩形与路径相交部分内希望放置的站点总数",
        update=update_station_plan,
    )
    x_min: FloatProperty(options={"HIDDEN"}, update=update_station_plan)
    x_max: FloatProperty(options={"HIDDEN"}, update=update_station_plan)
    y_min: FloatProperty(options={"HIDDEN"}, update=update_station_plan)
    y_max: FloatProperty(options={"HIDDEN"}, update=update_station_plan)
...
def _world_shell_directions(path_forward, phase_degrees=0.0):
    right, forward, up = _path_shell_frame(path_forward)
    if abs(float(phase_degrees)) > 1e-9:
        phase = Matrix.Rotation(math.radians(float(phase_degrees)), 4, up)
        right = (phase @ right).normalized()
        forward = (phase @ forward).normalized()
    directions = tuple(
...
def _surface_detail_camera_limit(settings, source_curve=None):
    if not bool(getattr(settings, "shell_surface_detail_enabled", True)):
        return 0
    count = max(0, int(getattr(settings, "shell_surface_detail_cameras", 4)))
    if source_curve is not None and (
        source_curve.get("gs_route_role", "") == "DETAIL"
        or source_curve.get("gs_detail_target_objects", "")
    ):
        count += 2
    return min(8, count)


def _surface_detail_direction_specs(
    scene, depsgraph, shell_center, path_forward, phase_degrees, settings, source_curve=None
):
    """Aim a few extra cameras at real occlusion boundaries around this station.

    The base icosahedron gives uniform angular coverage, but furniture gaps and
    recessed counters are defined by depth discontinuities rather than uniform
    directions. These probes find those discontinuities in the actual evaluated
    Blender scene. Each selected camera moves sideways from the target ray to add
    triangulation baseline, then looks back at the hit point.
    """
    maximum = _surface_detail_camera_limit(settings, source_curve)
    if maximum <= 0:
        return []
    center = Vector(shell_center)
    right, forward, up = _path_shell_frame(path_forward)
    if abs(float(phase_degrees)) > 1e-9:
        phase = Matrix.Rotation(math.radians(float(phase_degrees)), 4, up)
        right = (phase @ right).normalized()
        forward = (phase @ forward).normalized()
    units = _scene_units_per_meter(scene)
    search = max(0.5, float(getattr(settings, "shell_surface_probe_distance", 3.0))) * units
    minimum_jump = max(0.05, float(getattr(settings, "shell_surface_depth_jump", 0.30))) * units
    azimuth_count = 24
    records = []
    for pitch_degrees in (-15.0, 0.0, 15.0):
        pitch = math.radians(pitch_degrees)
        ring = []
        for index in range(azimuth_count):
            angle = math.tau * index / azimuth_count
            horizontal = (right * math.cos(angle) + forward * math.sin(angle)).normalized()
            direction = (horizontal * math.cos(pitch) + up * math.sin(pitch)).normalized()
            hit, point, normal, _face, obj, _matrix = _capture_ray_cast(
                scene, depsgraph, center + direction * 1e-4, direction, distance=search
            )
            if hit and obj:
                distance = (point - center).length
                ring.append({
                    "index": index, "direction": direction, "horizontal": horizontal,
                    "point": point.copy(), "normal": normal.copy(), "object": obj,
                    "distance": distance, "pitch_degrees": pitch_degrees,
                })
            else:
                ring.append(None)
        records.append(ring)
    ...
    ranked = []
    for ring in records:
        for index, record in enumerate(ring):
            if record is None or record["distance"] < 0.20 * units:
                continue
            neighbors = (ring[(index - 1) % azimuth_count], ring[(index + 1) % azimuth_count])
            jumps = [abs(record["distance"] - other["distance"]) for other in neighbors if other]
            if any(other is None for other in neighbors):
                jumps.append(max(0.0, search - record["distance"]))
            depth_jump = max(jumps, default=0.0)
            object_edge = any(other is None or other["object"] != record["object"] for other in neighbors)
            far_side = any(
                other is not None
                and record["distance"] > other["distance"] + minimum_jump
                for other in neighbors
            )
            facing = abs(record["normal"].normalized().dot(-record["direction"]))
            score = min(4.0, depth_jump / max(minimum_jump, 1e-8))
            score += 0.9 if object_edge else 0.0
            score += 1.5 if far_side else 0.0
            score += 0.5 * (1.0 - facing)
            score += 0.2 * max(0.0, 1.0 - record["distance"] / search)
            if record["pitch_degrees"] < 0.0:
                score += 0.55
            if record["point"].z < center.z - 0.08 * units:
                score += 0.45
            ranked.append((score, record))
    # When two rays form one edge, the farther hit is the useful view through the
    # opening; the nearer hit merely photographs the occluder again.
    ranked.sort(key=lambda item: (-item[0], -item[1]["distance"], item[1]["object"].name))

    selected = []

    def take(candidates, count, separation_degrees):
        minimum_separation = math.cos(math.radians(separation_degrees))
        for _score, record in candidates:
            if len(selected) >= maximum or count <= 0:
                break
            horizontal = record["horizontal"]
            if any(horizontal.dot(existing["horizontal"]) > minimum_separation for existing in selected):
                continue
            selected.append(record)
            count -= 1

    # Furniture gaps are commonly lost near the floor because high contrast wall
    # boundaries dominate a global ranking. Reserve half the budget for probes
    # that actually descend from the station into a lower surface or recess.
    low_ranked = [
        item for item in ranked
        if item[1]["pitch_degrees"] < 0.0
        and item[1]["point"].z < center.z - 0.08 * units
    ]
    take(low_ranked, max(1, maximum // 2), 24.0)
    minimum_separation = math.cos(math.radians(32.0))
    ...
    for index, record in enumerate(selected):
        ...
        primary_sign = -1.0 if (index + int(round(float(phase_degrees) / 5.0))) % 2 else 1.0
        is_low_gap = record["pitch_degrees"] < 0.0 and record["point"].z < center.z - 0.08 * units
        # A narrow opening may only be visible from one side of its foreground
        # edge. Emit a small left/right strafe pair for reserved low-gap targets.
        signs = (primary_sign, -primary_sign) if is_low_gap else (primary_sign,)
        for sign in signs:
            if len(specs) >= maximum:
                break
            if is_low_gap:
            # Lower the optical centre as well as the aim point. A merely downward
            # looking camera still sees the sofa/table slit from above and repeats
            # the failure mode found in out2.
                position_direction = (
                    horizontal * 0.25 + side * (0.86 * sign) - up * 0.44
                ).normalized()
                kind = "LOW_SURFACE_EDGE"
            else:
                position_direction = (horizontal * 0.35 + side * (0.93675 * sign)).normalized()
                kind = "SURFACE_EDGE"
            specs.append({
                "position_direction": position_direction,
                "target": record["point"].copy(),
                "kind": kind,
                "target_object": record["object"].name,
            })
        if len(specs) >= maximum:
            break
    return specs
...
def _shell_direction_specs(
    scene, depsgraph, shell_center, path_forward, phase_degrees, settings, source_curve=None
):
    directions, frame = _world_shell_directions(path_forward, phase_degrees)
    if source_curve is not None and source_curve.get("gs_peek_target"):
        raw=source_curve.get("gs_peek_targets",list(source_curve["gs_peek_target"]))
        targeted=[]
        for offset in range(0,min(len(raw),12),3):
            target=Vector(raw[offset:offset+3])
            toward=target-Vector(shell_center)
            if toward.length<1e-6:continue
            toward.normalize();side=toward.cross(Vector((0.,0.,1.)))
            if side.length<1e-6:side=frame[0].copy()
            side.normalize()
            targeted.extend({"position_direction":d.normalized(),"target":target,"kind":"DOORWAY_PEEK","target_object":""}
                            for d in (side,-side,toward*.45-Vector((0.,0.,.35))))
        return targeted or [{"position_direction":frame[0],"target":Vector(source_curve["gs_peek_target"]),"kind":"DOORWAY_PEEK"}],frame
    specs = [
        {"position_direction": direction, "view_direction": direction, "kind": "BASE"}
        for direction in directions
    ]
    specs.extend(_surface_detail_direction_specs(
        scene, depsgraph, shell_center, path_forward, phase_degrees, settings, source_curve
    ))
    return specs, frame


def _shell_spec_view_direction(spec, position):
    target = spec.get("target")
    if target is None:
        return Vector(spec.get("view_direction", spec["position_direction"])).normalized()
    direction = Vector(target) - Vector(position)
    if direction.length < 1e-8:
        return Vector(spec["position_direction"]).normalized()
...
def _capture_ray_cast(scene, depsgraph, origin, direction, distance):
    return capture_collision.ray_cast(scene, depsgraph, origin, direction, distance)
...
def _shell_radius_is_safe(scene, depsgraph, shell_center, directions, radius, clearance):
    for direction in directions:
        ...
        if not _shell_position_is_safe(scene, depsgraph, position, clearance):
            return False
        if radius > 2e-4:
            hit, _location, _normal, _face, obj, _matrix = _capture_ray_cast(
                scene, depsgraph,
...
def _shell_direction_radius_is_safe(scene, depsgraph, shell_center, direction, radius, clearance):
    position = shell_center + direction * radius
    if not _shell_position_is_safe(scene, depsgraph, position, clearance):
        return False
    if radius > 2e-4:
...
def _generate_spherical_shell_12_group(
    ...
    """Keep each safe shell camera independently; discard only blocked directions."""
    shell_center = Vector(shell_center)
    phase = (station_index - 1) * float(getattr(settings, "shell_station_rotation_step", 5.0))
    specs, frame = _shell_direction_specs(
        scene, depsgraph, shell_center, path_forward, phase, settings, source_curve
    )
    clearance = max(1e-6, float(getattr(settings, "shell_surface_clearance", 0.03))) * _scene_units_per_meter(scene)
    curve_name = getattr(source_curve, "name", "") if source_curve else ""
    cameras = []
    actual_radii = []
    for shell_index, spec in enumerate(specs):
        direction = Vector(spec["position_direction"]).normalized()
        accepted = None
        for candidate_m, candidate_scene in _shell_radius_candidates(scene, settings):
            if _shell_direction_radius_is_safe(
                scene, depsgraph, shell_center, direction, candidate_scene, clearance
            ):
                accepted = (candidate_m, candidate_scene)
                break
        if accepted is None:
            continue
        actual_radius_m, actual_radius_scene = accepted
        position = shell_center + direction * actual_radius_scene
        view_direction = _shell_spec_view_direction(spec, position)
        ...
        _orient_camera_radially(camera, view_direction, path_forward)
        _apply_path_camera_metadata(
            camera,
            station_index,
            shell_index,
        ...
        camera["shell_view_kind"] = spec.get("kind", "BASE")
        camera["shell_station_expected_count"] = len(specs)
        camera["shell_position_direction"] = tuple(direction)
        if spec.get("target_object"):
            camera["shell_target_object"] = spec["target_object"]
        if spec.get("target") is not None:
            camera["shell_target"] = tuple(spec["target"])
        cameras.append(camera)
        actual_radii.append(actual_radius_m)
    ...
    if not cameras:
        ...
        if use_fallback and _shell_position_is_safe(scene, depsgraph, shell_center, clearance):
            ...
            return cameras, [], True, False, None
        return [], [], False, True, None

    debug_radius = max(actual_radii) * _scene_units_per_meter(scene)
    return cameras, actual_radii, False, False, (shell_center.copy(), frame, debug_radius)
...
def path_component_counts(settings, components):
    from .station_budget import bounded_detail_counts
    return bounded_detail_counts(_base_path_component_counts(settings, components), components)


def active_station_regions(settings):
    if not getattr(settings, "station_plan_enabled", False):
        return []
    if getattr(settings, "path_capture_mode", "LEGACY_PANORAMA_CUBE") != "LEGACY_PANORAMA_CUBE":
        return []
    return [
        region for region in settings.station_regions
        if region.enabled and region.station_count > 0
        and abs(region.x_max - region.x_min) > 1e-6
        and abs(region.y_max - region.y_min) > 1e-6
    ]


def station_region_bounds(region):
    return (
        min(region.x_min, region.x_max), max(region.x_min, region.x_max),
        min(region.y_min, region.y_max), max(region.y_min, region.y_max),
    )


def point_in_station_region(point, region, epsilon=1e-7):
    x_min, x_max, y_min, y_max = station_region_bounds(region)
    return (
        x_min - epsilon <= point.x <= x_max + epsilon
        and y_min - epsilon <= point.y <= y_max + epsilon
    )


def station_regions_overlap(a, b, epsilon=1e-6):
    ax0, ax1, ay0, ay1 = station_region_bounds(a)
    bx0, bx1, by0, by1 = station_region_bounds(b)
    return min(ax1, bx1) - max(ax0, bx0) > epsilon and min(ay1, by1) - max(ay0, by0) > epsilon


def _clip_segment_to_station_region(start, end, region):
    x_min, x_max, y_min, y_max = station_region_bounds(region)
    dx = end.x - start.x
    dy = end.y - start.y
    t0, t1 = 0.0, 1.0
    for p, q in ((-dx, start.x - x_min), (dx, x_max - start.x),
                 (-dy, start.y - y_min), (dy, y_max - start.y)):
        if abs(p) <= 1e-12:
            if q < 0.0:
                return None
            continue
        ...
        if p < 0.0:
            ...
            if value < t0:
                return None
            t1 = min(t1, value)
    if t1 < t0 or (end - start).length * (t1 - t0) <= 1e-8:
        return None
    return start.lerp(end, t0), start.lerp(end, t1)


def _component_region_fragments(component, region):
    points = component["points"]
    fragments = []
    current = []
    for index in range(1, len(points)):
        clipped = _clip_segment_to_station_region(points[index - 1], points[index], region)
        if clipped is None:
            if len(current) > 1:
            ...
            continue
        start, end = clipped
        if current and (current[-1] - start).length <= 1e-6:
            current.append(end)
        else:
            if len(current) > 1:
                fragments.append(current)
            current = [start, end]
        if (end - points[index]).length > 1e-6:
            if len(current) > 1:
                fragments.append(current)
            current = []
    if len(current) > 1:
        fragments.append(current)
    return [
        {"object": component["object"], "points": points, "length": polyline_length(points)}
        for points in fragments if polyline_length(points) > 1e-8
    ]


def _sample_polyline_centred(points, count):
    if not points or count <= 0:
        return []
    if len(points) == 1:
        return [(points[0].copy(), Vector((0.0, 0.0, -1.0))) for _ in range(count)]
    lengths = [0.0]
    for index in range(1, len(points)):
        lengths.append(lengths[-1] + (points[index] - points[index - 1]).length)
    total = lengths[-1]
    ...
    segment = 1
    for item in range(count):
        distance = total * (item + 0.5) / count
        while segment < len(lengths) - 1 and lengths[segment] < distance:
            segment += 1
        start, end = points[segment - 1], points[segment]
        span = max(1e-9, lengths[segment] - lengths[segment - 1])
        factor = (distance - lengths[segment - 1]) / span
        tangent = end - start
        tangent = tangent.normalized() if tangent.length > 1e-8 else Vector((0.0, 0.0, -1.0))
        result.append((start.lerp(end, factor), tangent))
    return result


def sample_path_components_with_regions(settings, components):
    regions = active_station_regions(settings)
    if not regions:
        sampled = []
        for item, count in zip(components, path_component_counts(settings, components)):
            if count <= 0: continue
            samples = _sample_polyline_centred(item['points'], count) if count == 1 else sample_polyline_evenly(item['points'], count)
            for point, tangent in samples:
                sampled.append((point, tangent, item["object"]))
        return sampled

    from .station_budget import apportion, outside_fragments
    total = sum(path_component_counts(settings, components))
    fragments_by_region = [[fragment for component in components
                           for fragment in _component_region_fragments(component, region)] for region in regions]
    requested = [region.station_count if fragments else 0 for region, fragments in zip(regions, fragments_by_region)]
    quotas = apportion(total, requested) if sum(requested) > total else requested
    sampled = []
    for fragments, count in zip(fragments_by_region, quotas):
        if not fragments or count <= 0: continue
        for fragment, n in zip(fragments, distribute_counts_by_length(count, fragments)):
            for point, tangent in _sample_polyline_centred(fragment['points'], n):
                sampled.append((point, tangent, fragment['object']))
    remaining = total - len(sampled)
    outside = outside_fragments(components, regions, _clip_segment_to_station_region)
    if remaining > 0 and outside:
        for fragment, count in zip(outside, distribute_counts_by_length(remaining, outside)):
            for point, tangent in _sample_polyline_centred(fragment['points'], count):
                sampled.append((point, tangent, fragment['object']))
    return sampled
...
def sample_path_components(settings):
    ...
    return sample_path_components_with_regions(settings, components)


def station_plan_report(settings):
    regions = active_station_regions(settings)
    sampled = sample_path_components(settings)
    counts = [0] * len(regions)
    for point, _tangent, _obj in sampled:
        for index, region in enumerate(regions):
            if point_in_station_region(point, region):
                counts[index] += 1
                break
    return {"total": len(sampled), "regions": counts, "requested": [r.station_count for r in regions]}
...
def _nearest_surface(scene, depsgraph, point, dirs, search):
    ...
    farthest = -1.0
    for d in dirs:
        hit, loc, _n, _fi, _ob, _mx = _capture_ray_cast(scene, depsgraph, point + d * 1e-4, d, distance=search)
...
def create_rig(scene, settings):
    token = capture_collision.begin(scene, settings)
    try:
        return _create_rig_impl(scene, settings)
    finally:
        capture_collision.end(token)


def _create_rig_impl(scene, settings):
        ...
        if settings.path_capture_mode == "SCIENTIFIC_THREE_LAYER":
            ...
            if shell_path:
                ...
                for index, (point, tangent, path_object) in enumerate(sampled):
                    ...
                    if station_radii:
                        actual_radii.extend(station_radii)
                        expected = int(group[0].get("shell_station_expected_count", 12)) if group else 12
                        filtered_camera_count += max(0, expected - len(station_radii))
                        if any(radius + 1e-9 < requested_radius for radius in station_radii):
                            adaptive_shrunk += 1
                ...
                _store_path_camera_rig_report(scene, {
                    "mode": "SPHERICAL_SHELL_12_SURFACE_DETAIL" if getattr(settings, "shell_surface_detail_enabled", True) else "SPHERICAL_SHELL_12",
                    "station_count": path_station_count,
                    "cameras_per_station": 12,
                    "maximum_cameras_per_station": 12 + _surface_detail_camera_limit(settings),
                    "surface_detail_camera_count": sum(
                        1 for camera in cameras if str(camera.get("shell_view_kind", "")).endswith("SURFACE_EDGE")
                    ),
                    ...
                    "filtered_camera_count": filtered_camera_count,
...
def training_consistency_report(scene, settings, cameras):
    ...
    return {
        ...
        "scene_exposure": float(scene.view_settings.exposure),
...
def _sample_sparse_points_impl(scene, cameras, settings, progress_callback=None):
    ...
    if progress_callback is not None:
        progress_callback(0, total_views)
    for view_index, camera in enumerate(cameras, start=1):
        ...
        for ix in range(per_side):
            ...
            for iy in range(per_side):
                ...
                hit, location, normal, face_index, obj, matrix = _capture_ray_cast(scene, depsgraph, origin, direction, distance=settings.ray_distance)
        ...
        if progress_callback is not None:
            progress_callback(view_index, total_views)
    return list(points.values())


def sample_sparse_points(scene, cameras, settings, progress_callback=None):
    """Sample the scene once per camera ray grid using one shared geometry BVH."""
    visual_states = hide_camera_mesh_visuals(scene)
    token = capture_collision.begin(scene, settings)
    try:
        return _sample_sparse_points_impl(scene, cameras, settings, progress_callback)
    finally:
        capture_collision.end(token)
...
def configure_render(scene, settings, gpu_restrict=None, apply_color=True):
    enforce_simple_outputs(settings)
    active_backend = "CPU"
...
def _render_pose_sequence_dataset(
    ...
    def point_progress(current, view_total):
        write_progress(
            progress_path, "points", current, view_total,
            f"Sampling sparse point cloud: {current}/{view_total}", elapsed=render_seconds,
        )

    points = sample_sparse_points(scene, export_cameras, settings, point_progress)
...
def _camera_planning_report(scene, settings, cameras):
    ...
    return {
        ...
        "layer_count": 1,
        "layer_camera_counts": {"spherical_shell" if rig_mode.startswith("SPHERICAL_SHELL_12") else "shared_origin": len(cameras)},
...
def write_report(scene, settings, cameras, image_names, points):
    ...
    report = {
        ...
        "color_exposure": float(scene.view_settings.exposure),
...
SETTINGS_VALUE_KEYS = (
    ...
    "shell_radius_mode",
    "shell_min_radius",
    "shell_surface_clearance",
    "shell_surface_detail_enabled",
    "shell_surface_detail_cameras",
    "shell_surface_probe_distance",
    "shell_surface_depth_jump",
    "shell_station_rotation_step",
...
def update_contour_region(self, context):
    obj=self.contour_region_object
    if obj and obj.type=='MESH':
        obj['gs_path_region']=True
...
_SIMPLE_OUTPUT_KEYS=("render_rgb","export_depth","export_object_depth","export_normal","export_object_normal","export_object_mask","export_id","export_material_id")


def enforce_simple_outputs(settings):
    if getattr(settings,"interface_mode","SIMPLE")!="SIMPLE":return
    if "gs_professional_outputs" not in settings:
        settings["gs_professional_outputs"]=json.dumps({k:getattr(settings,k) for k in _SIMPLE_OUTPUT_KEYS})
    for key in _SIMPLE_OUTPUT_KEYS:setattr(settings,key,key=="render_rgb")


def update_interface_mode(self,context):
    if self.interface_mode=="SIMPLE":enforce_simple_outputs(self)
    elif self.get("gs_professional_outputs"):
        saved=json.loads(self["gs_professional_outputs"])
        for key,value in saved.items():setattr(self,key,value)
        del self["gs_professional_outputs"]
    tag_view3d_redraw()


class GSCOLMAP_Settings(PropertyGroup):
    interface_mode: EnumProperty(name="界面模式",items=(("SIMPLE","精简模式","路径、12 方位相机与 RGB 渲染"),("PRO","专业模式","全部规划与渲染参数")),default="SIMPLE",update=update_interface_mode)
    ...
    station_plan_enabled: BoolProperty(
        name="启用站点配额平面图", default=False,
        description="开启后，框选区域的站点配额参与路径采样；关闭时完全沿用原站点算法",
        update=update_station_plan,
    )
    station_plan_view_active: BoolProperty(
        name="站点平面图显示", default=False, options={"SKIP_SAVE"},
    )
    station_regions: CollectionProperty(type=GSCOLMAP_StationRegion)
    station_region_active_index: IntProperty(default=0, min=0)
    station_region_default_count: IntProperty(
        name="新区域站点数", default=24, min=1, max=10000,
    )
    station_plan_marker_size: FloatProperty(
        name="站点标记大小", default=6.0, min=1.0, max=20.0,
        update=update_station_plan,
    )
    station_plan_resolution: IntProperty(
        name="CAD 底图分辨率", default=1024, min=512, max=2048,
    )
    station_plan_x_min: FloatProperty(options={"HIDDEN"})
    station_plan_x_max: FloatProperty(options={"HIDDEN"})
    station_plan_y_min: FloatProperty(options={"HIDDEN"})
    station_plan_y_max: FloatProperty(options={"HIDDEN"})
    camera_overlay_enabled: BoolProperty(
        name="显示轻量相机代理", default=True,
        description="在普通视图显示所有路径相机的 GPU 点和朝向线；孤立模式下仅显示当前路径",
        update=update_path_inspection_overlay,
    )
    path_inspection_enabled: BoolProperty(
        name="单路径检查模式", default=False, options={"SKIP_SAVE"},
        description="隐藏其他排线路径和真实相机图标，用轻量 GPU 标记检查当前路径的相机排布",
    )
    path_inspection_curve: PointerProperty(
        name="检查路径", type=bpy.types.Object, options={"SKIP_SAVE"},
    )
    path_inspection_marker_size: FloatProperty(
        name="相机点大小", default=5.0, min=1.0, max=16.0,
        description="GPU 相机位置标记的屏幕像素大小",
        update=update_path_inspection_overlay,
    )
    path_inspection_ray_length: FloatProperty(
        name="朝向线长度 (m)", default=0.12, min=0.01, soft_max=1.0,
        description="每台相机的橙色镜头朝向线长度",
        update=update_path_inspection_overlay,
    )
    path_inspection_xray: BoolProperty(
        name="穿透显示", default=False,
        description="让相机代理透过家具和墙体保持可见",
        update=update_path_inspection_overlay,
    )
    ...
    shell_surface_clearance: FloatProperty(
        name="球壳相机表面净距 (m)",
        description="只剔除贴近或穿过地面、墙面和物体表面的单台球壳相机",
        default=0.03,
        min=0.005,
        max=0.20,
        precision=3,
    ...
    shell_surface_detail_enabled: BoolProperty(
        name="场景表面补视角",
        description="探测真实网格的遮挡边缘和凹位，在基础 12 方位之外追加少量定向相机",
        default=True,
        update=live_update_camera_rig,
    )
    shell_surface_detail_cameras: IntProperty(
        name="每站补视角上限",
        description="普通路径每站最多追加的表面细节相机；细部短线最多再增加 2 台",
        default=4,
        min=0,
        max=8,
        update=live_update_camera_rig,
    )
    shell_surface_probe_distance: FloatProperty(
        name="表面探测距离 (m)",
        description="从路径站点向周围探测家具、墙面和凹位的最大距离",
        default=3.0,
        min=0.5,
        max=8.0,
        precision=2,
    ...
    shell_surface_depth_jump: FloatProperty(
        name="遮挡深度突变阈值 (m)",
        description="相邻探测射线超过该深度差时视为值得补拍的家具边缘或空间开口",
        default=0.30,
        min=0.05,
        max=2.0,
        precision=2,
    ...
    shell_station_rotation_step: FloatProperty(
        name="每站旋转角度 (°)",
        description="相邻路径站点的球壳阵列绕竖直轴增加的旋转角度；0 表示不递增",
        default=5.0,
        min=0.0,
        max=45.0,
        precision=1,
    ...
    contour_region_enabled: BoolProperty(name="启用区域排线",default=False,
        description="将轮廓主线和所有补线限制在指定封闭网格的实际内部")
    contour_region_object: PointerProperty(name="区域网格",type=bpy.types.Object,
        poll=lambda self,obj:obj.type=='MESH',update=update_contour_region,
        description="选中的网格将用作线框区域辅助物，不参与渲染和避障")
    contour_region_size: FloatProperty(name="初始区域边长（m）",default=3.,min=.1,soft_max=20.)
    ...
    contour_camera_audit_enabled: BoolProperty(name="按现有相机检查整体表面缺口", default=True)
    contour_audit_budget: IntProperty(name="表面缺口补线数量", default=16, min=1, max=60)
    contour_peek_enabled: BoolProperty(name="门洞直线探入补线", default=True)
    contour_peek_budget: IntProperty(name="探入短线数量上限", default=24, min=1, max=80)
    contour_peek_depth: FloatProperty(name="最大探入深度 (m)", default=1.2, min=.4, max=3.)
    coverage_heat_enabled: BoolProperty(name="覆盖颜色",default=False,update=lambda s,c: coverage_heat.redraw())
    coverage_busy: BoolProperty(default=False,options={'SKIP_SAVE'})
    coverage_progress: FloatProperty(name="覆盖检测进度",default=0.,min=0.,max=1.,options={'SKIP_SAVE'})
    coverage_status: StringProperty(default="先检测覆盖，再刷选缺失区域",options={'SKIP_SAVE'})
    coverage_camera_collection: PointerProperty(name="检测相机集合（空=全场景）",type=bpy.types.Collection)
    coverage_surface_collection: PointerProperty(name="检测表面集合（空=可见场景）",type=bpy.types.Collection)
    coverage_spacing: FloatProperty(name="表面采样间距 (m)",default=.35,min=.05,max=2.)
    coverage_good_views: IntProperty(name="充足独立视角数",default=3,min=2,max=8)
    coverage_min_distance: FloatProperty(name="最小观察距离 (m)",default=.35,min=.1,max=2.)
    coverage_max_distance: FloatProperty(name="最大观察距离 (m)",default=4.,min=.5,max=20.)
    coverage_incidence: FloatProperty(name="最大表面入射角 (度)",default=75.,min=20.,max=85.)
    coverage_baseline: FloatProperty(name="最小相机基线 (m)",default=.12,min=.03,max=1.)
    coverage_angle: FloatProperty(name="最小独立观察夹角 (度)",default=10.,min=2.,max=35.)
    coverage_safety: FloatProperty(name="补拍相机净距 (m)",default=.15,min=.05,max=1.)
    coverage_overlap: FloatProperty(name="最小共同表面比例",default=.25,min=.1,max=.8)
    coverage_patch_budget: IntProperty(name="刷选补拍数量上限",default=24,min=1,max=100)
    coverage_repair_enhance: BoolProperty(name="补强已覆盖区域",default=False,
        description="允许为刷选的绿色表面补充独立视角；绿色只表示达到覆盖阈值，不保证重建细节质量。仍检查距离、遮挡、净空和重叠")
    coverage_repair_extra_views: IntProperty(name="额外独立视角目标",default=2,min=1,max=6,
        description="相对当前已有独立视角，为绿色刷选表面尝试增加的视角数；受补拍数量上限和可用机位约束")
    coverage_brush_radius: FloatProperty(name="表面笔刷半径 (m)",default=.35,min=.05,max=3.)
    contour_ceiling_detail_enabled: BoolProperty(name="挑空顶部细部补线",default=True,description="在现有顶层以上寻找安全净空位置，并按可见性与空间缺口选择短线")
    ...
    contour_cross_grid_enabled: BoolProperty(name="叠加交叉井字骨架", default=True)
    contour_cross_grid_spacing: FloatProperty(name="井字骨架间距 (m)", default=1.2,min=.4,max=4.)
    contour_cross_grid_budget: IntProperty(name="每层骨架线数量上限", default=12,min=2,max=60)
    contour_progress: FloatProperty(name="路径生成进度",default=0.,min=0.,max=1.,options={"SKIP_SAVE"})
...
class GSCOLMAP_OT_create_cameras(Operator):
    ...
    def _finish(self, context):
        ...
        capture_collision.end(self._capture_scope)
    ...
    def _prepare(self, context):
        ...
        self._capture_scope = capture_collision.begin(scene, settings)
    ...
    def _begin_station_probe(self, context):
        ...
        phase = self._index * float(getattr(settings, "shell_station_rotation_step", 5.0))
        specs, frame = _shell_direction_specs(
            scene, self._depsgraph, point, path_forward, phase, settings, path_object
        )
        directions = tuple(Vector(spec["position_direction"]).normalized() for spec in specs)
        clearance = max(1e-6, float(getattr(settings, "shell_surface_clearance", 0.03))) * _scene_units_per_meter(scene)
        state = {
            "center": Vector(point),
    ...
    def _record_station_result(
        self, context, group, actual_radii, used_fallback, skipped, debug_spec, expected_count=12
    ):
        total = len(self._sampled)
        self._mark_final_names(group)
        self._cameras.extend(group)
        actual_radii = list(actual_radii)
        if actual_radii:
            self._actual_radii.extend(actual_radii)
            self._filtered_camera_count += max(0, int(expected_count) - len(actual_radii))
            if any(radius + 1e-9 < self._requested_radius for radius in actual_radii):
    ...
    def _create_safe_shell_camera(self, context, state, actual_radius_m, actual_radius_scene):
        settings = context.scene.gs_colmap_settings
        curve = state["source_curve"]
        curve_name = getattr(curve, "name", "") if curve else ""
        shell_index = state["direction_index"]
        direction = state["directions"][shell_index]
        spec = state["specs"][shell_index]
        position = state["center"] + direction * actual_radius_scene
        view_direction = _shell_spec_view_direction(spec, position)
        camera = create_camera_at(
        ...
        _orient_camera_radially(camera, view_direction, state["path_forward"])
        _apply_path_camera_metadata(
            camera,
            self._index + 1,
            shell_index,
        ...
        camera["shell_view_kind"] = spec.get("kind", "BASE")
        camera["shell_station_expected_count"] = len(state["specs"])
        camera["shell_position_direction"] = tuple(direction)
        if spec.get("target_object"):
            camera["shell_target_object"] = spec["target_object"]
        if spec.get("target") is not None:
            camera["shell_target"] = tuple(spec["target"])
        state["group"].append(camera)
        state["actual_radii"].append(actual_radius_m)
        self._advance_shell_direction(context, state)
    ...
    def _finish_unsafe_station(self, context, state, fallback_safe):
        ...
        if use_fallback and fallback_safe:
            ...
            self._record_station_result(
                context, group, [], True, False, None, len(state["directions"])
            )
        else:
            self._record_station_result(
                context, [], [], False, True, None, len(state["directions"])
            )

    def _advance_shell_direction(self, context, state):
        ...
        if state["direction_index"] < len(state["directions"]):
            ...
            self._start_position_probe(state, state["center"] + direction * candidate_scene, "shell")
            return
        if state["group"]:
            debug_radius = max(state["actual_radii"]) * _scene_units_per_meter(context.scene)
            debug_spec = (state["center"].copy(), state["frame"], debug_radius)
            self._record_station_result(
                context, state["group"], state["actual_radii"], False, False, debug_spec,
    ...
    def _position_probe_passed(self, context, state):
        ...
        if candidate_scene > 2e-4:
            ...
            if hit:
                ...
                return
        self._create_safe_shell_camera(context, state, candidate_m, candidate_scene)
    ...
    def _generate_station_slice(self, context):
        ...
        while self._station_state is state and rays < ray_budget and time.monotonic() - started < 0.015:
            ...
            hit, location, _normal, _face, _obj, _matrix = _capture_ray_cast(
                context.scene, self._depsgraph,
        ...
        if self._station_state is state:
            ...
            direction_count = max(1, len(state["directions"]))
            direction_number = min(direction_count, state["direction_index"] + 1)
            subprogress = state["direction_index"] / float(direction_count)
            fraction = 0.05 + 0.70 * (self._index + subprogress) / max(1, total_stations)
            mode = "回退安全检测" if state["position_mode"] == "fallback" else (
                f"半径 {candidate_number}/{candidate_count} · 位置 {direction_number}/{direction_count}"
            )
            self._set_progress(
                context,
                fraction,
    ...
    def _begin_commit(self, context):
        ...
        if not self._cameras:
            raise RuntimeError(
                f"{len(self._sampled)} 个站点均未通过球壳或回退安全检查；"
                "请减小最小净距/球壳半径，或检查失败处理设置"
            )
        settings.camera_build_cancelable = False
    ...
    def _store_report(self, context):
        ...
        _store_path_camera_rig_report(context.scene, {
            "mode": "SPHERICAL_SHELL_12_SURFACE_DETAIL" if getattr(context.scene.gs_colmap_settings, "shell_surface_detail_enabled", True) else "SPHERICAL_SHELL_12",
            "station_count": len(self._sampled),
            "cameras_per_station": 12,
            "maximum_cameras_per_station": 12 + _surface_detail_camera_limit(context.scene.gs_colmap_settings),
            "surface_detail_camera_count": sum(
                1 for camera in self._cameras if str(camera.get("shell_view_kind", "")).endswith("SURFACE_EDGE")
            ),
            ...
            "filtered_camera_count": self._filtered_camera_count,
    ...
    def _complete(self, context):
        ...
        invalidate_camera_overlay(context.scene)
        settings = context.scene.gs_colmap_settings
...
def _path_collection_curves(settings, selected_curve):
    curves = collection_curve_objects(settings.path_collection) if settings.path_collection else []
    if selected_curve not in curves:
        curves.append(selected_curve)
    return curves


def _generated_path_cameras(scene, settings):
    camera_collection = bpy.data.collections.get(settings.camera_collection)
    collection_names = {
        obj.name for obj in camera_collection.all_objects if obj.type == "CAMERA"
    } if camera_collection else set()
    return [
        obj for obj in scene.objects
        if obj.type == "CAMERA" and (obj.get("source_curve", "") or obj.name in collection_names)
    ]


def _restore_path_inspection(scene):
    state = _PATH_INSPECTION_STATES.pop(scene.as_pointer(), None)
    if state:
        for name, hidden in state["hidden"].items():
            obj = bpy.data.objects.get(name)
            if obj is not None and scene.objects.get(obj.name) is not None:
                try:
                    obj.hide_set(hidden)
                except RuntimeError:
                    pass
        curve = bpy.data.objects.get(state.get("curve_name", ""))
        if curve is not None:
            curve.show_in_front = state.get("curve_show_in_front", False)
    if hasattr(scene, "gs_colmap_settings"):
        settings = scene.gs_colmap_settings
        settings.path_inspection_enabled = False
        settings.path_inspection_curve = None
        sync_native_camera_sizes(scene,settings.camera_overlay_enabled)
    invalidate_camera_overlay(scene)
    tag_view3d_redraw()
    return state is not None


class GSCOLMAP_OT_toggle_path_inspection(Operator):
    bl_idname = "gs_colmap.toggle_path_inspection"
    bl_label = "单路径相机检查"
    bl_description = "保留完整场景，仅隐藏其他排线路径和原生相机图标，并显示轻量 GPU 相机代理"

    def execute(self, context):
        scene = context.scene
        settings = scene.gs_colmap_settings
        if settings.path_inspection_enabled:
            restored = _restore_path_inspection(scene)
            self.report({"INFO"}, "已退出单路径检查并恢复此前显示状态" if restored else "已退出单路径检查")
            return {"FINISHED"}

        curve = context.active_object
        if curve is None or curve.type != "CURVE":
            self.report({"ERROR"}, "请先选中一条路径曲线")
            return {"CANCELLED"}
        if curve.name not in context.view_layer.objects:
            self.report({"ERROR"}, "所选路径不在当前视图层")
            return {"CANCELLED"}

        curves = [obj for obj in _path_collection_curves(settings, curve)
                  if obj.name in context.view_layer.objects]
        cameras = [obj for obj in _generated_path_cameras(scene, settings)
                   if obj.name in context.view_layer.objects]
        hidden = {}
        for obj in curves + cameras:
            if obj.name not in hidden:
                hidden[obj.name] = bool(obj.hide_get())
        _PATH_INSPECTION_STATES[scene.as_pointer()] = {
            "hidden": hidden,
            "curve_name": curve.name,
            "curve_show_in_front": bool(curve.show_in_front),
        }

        # The old mesh pyramid feature is retired. Remove any objects left by
        # earlier add-on versions before entering the zero-object overlay mode.
        cleanup_legacy_camera_mesh_visuals()
        sync_native_camera_sizes(scene,settings.camera_overlay_enabled)
        for path in curves:
            path.hide_set(path != curve)
        for camera in cameras:
            camera.hide_set(True)
        curve.hide_set(False)
        curve.show_in_front = True
        settings.path_inspection_curve = curve
        settings.path_inspection_enabled = True
        invalidate_camera_overlay(scene)
        target_count = len(path_inspection_cameras(scene, curve))
        tag_view3d_redraw()
        self.report({"INFO"}, f"正在检查 {curve.name}：{target_count} 台相机（Alt+Shift+I 退出）")
        return {"FINISHED"}


class GSCOLMAP_OT_add_doorway_peeks(Operator):
    bl_idname = "gs_colmap.add_doorway_peeks"
    bl_label = "检查相机覆盖并补门洞及表面缺口"
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        from . import doorway_peek, surface_audit
        wm=context.window_manager
        wm.progress_begin(0,100)
        try:
            return self._execute(context,doorway_peek,surface_audit,
                                 lambda label,current,total:wm.progress_update(100*current/max(1,total)))
        finally:
            wm.progress_end()

    def _execute(self,context,doorway_peek,surface_audit,progress):
        scene=context.scene; settings=scene.gs_colmap_settings
        scale=max(1e-9,scene.unit_settings.scale_length)
        routes=[]
        for obj in path_curve_objects(settings):
            if obj.get("gs_peek_route"):continue
            points=[tuple(p*scale) for p in curve_polyline_points(obj, settings.path_samples_per_segment)]
            if len(points)>1:routes.append((int(obj.get("gs_capture_layer_index",0)),str(obj.get("gs_floorplan_layer","Path")),contour_blender.planner.Route(points)))
        if not routes:
            self.report({"ERROR"},"请在路径集合中指定保留曲线")
            return {"CANCELLED"}
        probe=contour_blender.SceneProbe(scene,scale,min(settings.contour_clearance,.12),.15,
                                      region=region_volume.active_region(scene,settings))
        probe.vertical_range=20.
        existing_cameras=_generated_path_cameras(scene,settings)
        candidate_budget=settings.contour_peek_budget*(2 if settings.contour_camera_audit_enabled and existing_cameras else 1)
        found,report=doorway_peek.discover(routes,probe,candidate_budget if settings.contour_peek_enabled else 0,settings.contour_peek_depth,progress)
        if settings.contour_camera_audit_enabled and existing_cameras:
            camera_routes,audit=surface_audit.audit_and_refine(scene,routes,probe,existing_cameras,settings.contour_audit_budget,progress)
            found.extend(camera_routes);report["camera_surface_audit"]=audit
            found,verification=surface_audit.validate_routes(scene,settings,probe,found,audit,progress,settings.contour_peek_budget+settings.contour_audit_budget,peek_budget=settings.contour_peek_budget)
            report["actual_camera_verification"]=verification
        report["accepted"]=len(found)
        report["length_m"]=sum(contour_blender.planner.length(r.points) for _,_,r in found)
        report["paths"]=[dict(layer=label,points=r.points,target=r.peek_target,targets=getattr(r,"peek_targets",[r.peek_target])) for _,label,r in found]
        if not found:
            scene["gs_doorway_peek_report"]=json.dumps(report,ensure_ascii=False)
            self.report({"INFO"},"未找到通过安全与视角收益检查的新增路线")
            return {"FINISHED"}
        collection=bpy.data.collections.new("GS_Doorway_Peek")
        if settings.path_collection:settings.path_collection.children.link(collection)
        else:scene.collection.children.link(collection)
        for index,(layer,label,route) in enumerate(found):
            data=bpy.data.curves.new(f"GS_Peek_{index+1:03d}","CURVE");data.dimensions="3D";data.bevel_depth=.006/scale
            sp=data.splines.new("POLY");sp.points.add(len(route.points)-1)
            for pt,p in zip(sp.points,route.points):pt.co=tuple(v/scale for v in p)+(1.,)
            obj=bpy.data.objects.new(data.name,data);collection.objects.link(obj);obj.hide_render=True;obj.color=(1.,.15,.02,1.)
            obj["gs_peek_route"]=True;obj["gs_peek_target"]=tuple(v/scale for v in route.peek_target)
            if getattr(route,"peek_targets",None):obj["gs_peek_targets"]=[v/scale for p in route.peek_targets for v in p]
            obj["gs_route_role"]="DETAIL";obj["gs_contour_version"]="1.6.8";obj["gs_detail_min_samples"]=5
            obj["gs_layer_index"]=layer;obj["gs_layer_label"]=label
            obj["gs_explicit_capture_height"]=True;obj["gs_capture_layer_index"]=layer;obj["gs_floorplan_layer"]=label
        scene["gs_doorway_peek_report"]=json.dumps(report,ensure_ascii=False)
        self.report({"INFO"},f"新增 {len(found)} 条通过检查的覆盖补线，请重新生成相机")
        return {"FINISHED"}


class GSCOLMAP_OT_refresh_path_inspection(Operator):
    bl_idname = "gs_colmap.refresh_path_inspection"
    bl_label = "刷新轻量相机代理"
    bl_description = "相机位置或朝向改变后重建当前 GPU 叠加层"

    def execute(self, context):
        settings = context.scene.gs_colmap_settings
        if not (settings.path_inspection_enabled or settings.camera_overlay_enabled):
            self.report({"ERROR"}, "请先开启轻量相机代理")
            return {"CANCELLED"}
        invalidate_camera_overlay(context.scene)
        count = len(path_inspection_cameras(context.scene, settings.path_inspection_curve) if settings.path_inspection_enabled else _generated_path_cameras(context.scene, settings))
        tag_view3d_redraw()
        self.report({"INFO"}, f"已刷新 {count} 台相机代理")
        return {"FINISHED"}


class GSCOLMAP_UL_station_regions(UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        region = item
        row = layout.row(align=True)
        row.prop(region, "enabled", text="")
        row.prop(region, "name", text="", emboss=False, icon="SELECT_SET")
        row.label(text=f"{region.station_count} 站")


def _restore_station_plan_view(scene):
    global _STATION_PLAN_PREVIEW
    state = _STATION_PLAN_VIEW_STATES.pop(scene.as_pointer(), None)
    if state:
        area = state.get("area")
        if area is not None:
            try:
                area.type = state.get("area_type", "VIEW_3D")
            except ReferenceError:
                pass
    if hasattr(scene, "gs_colmap_settings"):
        scene.gs_colmap_settings.station_plan_view_active = False
    _STATION_PLAN_PREVIEW = None
    invalidate_station_plan(scene)
    tag_view3d_redraw()


class GSCOLMAP_OT_toggle_station_plan_view(Operator):
    bl_idname = "gs_colmap.toggle_station_plan_view"
    bl_label = "站点配额平面图"
    bl_description = "生成 CAD 风格平面底图并打开独立站点规划图像工作区"

    def execute(self, context):
        scene = context.scene
        settings = scene.gs_colmap_settings
        if settings.station_plan_view_active:
            _restore_station_plan_view(scene)
            return {"FINISHED"}
        if context.area is None or context.area.type != "VIEW_3D":
            self.report({"ERROR"}, "请在 3D 视图的 N 面板中生成 CAD 平面图")
            return {"CANCELLED"}
        if not settings.station_plan_enabled:
            self.report({"ERROR"}, "请先开启“启用站点配额平面图”")
            return {"CANCELLED"}
        curves = [obj for obj in path_curve_objects(settings) if obj.name in context.view_layer.objects]
        if not curves:
            self.report({"ERROR"}, "请先生成或指定路径曲线")
            return {"CANCELLED"}
        _STATION_PLAN_VIEW_STATES[scene.as_pointer()] = {
            "area": context.area,
            "area_type": context.area.type,
        }
        try:
            image = render_station_cad_plan(context)
            context.area.type = "IMAGE_EDITOR"
            context.area.spaces.active.image = image
            settings.station_plan_view_active = True
            try:
            ...
            invalidate_station_plan(scene)
            tag_view3d_redraw()
            self.report({"INFO"}, "CAD 平面图已生成；蓝线为路径，粉色点为框内配额站点")
            return {"FINISHED"}
        except Exception as exc:
            _restore_station_plan_view(scene)
            self.report({"ERROR"}, f"生成 CAD 平面图失败：{exc}")
            return {"CANCELLED"}


class GSCOLMAP_OT_draw_station_region(Operator):
    bl_idname = "gs_colmap.draw_station_region"
    bl_label = "框选站点配额区域"
    bl_description = "先输入站点数，再直接在 CAD 平面图上按住左键拖出矩形"
    bl_options = {"REGISTER", "UNDO"}

    station_count: IntProperty(name="区域目标站点数", default=24, min=1, max=10000)

    def invoke(self, context, event):
        settings = context.scene.gs_colmap_settings
        if context.area is None or context.area.type != "IMAGE_EDITOR" or not settings.station_plan_view_active:
            self.report({"ERROR"}, "请先生成并打开 CAD 站点平面图")
            return {"CANCELLED"}
        ...
        if self._window_region is None:
            return {"CANCELLED"}
        self.station_count = settings.station_region_default_count
        return context.window_manager.invoke_props_dialog(self, width=320)

    def draw(self, context):
        self.layout.prop(self, "station_count")
        self.layout.label(text="确认后直接在 CAD 平面图上按住左键拖框")
    ...
    def execute(self, context):
        ...
        self.report({"INFO"}, "按住左键拖出区域，Esc 取消")
        return {"RUNNING_MODAL"}

    def _finish_cursor(self, context):
        global _STATION_PLAN_PREVIEW
        _STATION_PLAN_PREVIEW = None
    ...
    def modal(self, context, event):
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS":
            self._start = _station_image_mouse_world(
                context, event, context.scene.gs_colmap_settings, self._window_region
            )
            self._current = self._start
            return {"RUNNING_MODAL"}
        if event.type == "MOUSEMOVE" and self._start is not None:
            self._current = _station_image_mouse_world(
                context, event, context.scene.gs_colmap_settings, self._window_region
            )
            if self._current is not None:
                _STATION_PLAN_PREVIEW = (
        ...
        if event.type == "LEFTMOUSE" and event.value == "RELEASE" and self._start is not None:
            end = _station_image_mouse_world(
                context, event, context.scene.gs_colmap_settings, self._window_region
            )
            if end is None:
                end = self._current
            if end is None or abs(end.x - self._start.x) < 1e-5 or abs(end.y - self._start.y) < 1e-5:
                self._finish_cursor(context)
                self.report({"ERROR"}, "区域太小，请重新拖框")
                return {"CANCELLED"}
            settings = context.scene.gs_colmap_settings
            candidate = type("Bounds", (), {
                "x_min": min(self._start.x, end.x), "x_max": max(self._start.x, end.x),
                "y_min": min(self._start.y, end.y), "y_max": max(self._start.y, end.y),
            })()
            if any(station_regions_overlap(candidate, region) for region in active_station_regions(settings)):
                self._finish_cursor(context)
                self.report({"ERROR"}, "配额区域不能重叠，请重新划分边界")
                return {"CANCELLED"}
            region = settings.station_regions.add()
            region.name = f"配额区域 {len(settings.station_regions)}"
            region.station_count = self.station_count
            region.x_min, region.x_max = candidate.x_min, candidate.x_max
            region.y_min, region.y_max = candidate.y_min, candidate.y_max
            settings.station_region_active_index = len(settings.station_regions) - 1
            settings.station_region_default_count = self.station_count
            self._finish_cursor(context)
            invalidate_station_plan(context.scene)
            report = station_plan_report(settings)
            actual = report["regions"][len(active_station_regions(settings)) - 1]
            self.report({"INFO"}, f"已创建区域：目标 {self.station_count}，当前预计 {actual} 个站点")
            return {"FINISHED"}
        return {"RUNNING_MODAL"}


class GSCOLMAP_OT_remove_station_region(Operator):
    bl_idname = "gs_colmap.remove_station_region"
    bl_label = "删除站点区域"
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        settings = context.scene.gs_colmap_settings
        index = settings.station_region_active_index
        if not (0 <= index < len(settings.station_regions)):
            return {"CANCELLED"}
        settings.station_regions.remove(index)
        settings.station_region_active_index = max(0, min(index, len(settings.station_regions) - 1))
        invalidate_station_plan(context.scene)
        tag_view3d_redraw()
        return {"FINISHED"}


class GSCOLMAP_OT_clear_station_regions(Operator):
    bl_idname = "gs_colmap.clear_station_regions"
    bl_label = "清空站点区域"
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        context.scene.gs_colmap_settings.station_regions.clear()
        context.scene.gs_colmap_settings.station_region_active_index = 0
        invalidate_station_plan(context.scene)
        tag_view3d_redraw()
        return {"FINISHED"}


class GSCOLMAP_OT_refresh_station_plan(Operator):
    bl_idname = "gs_colmap.refresh_station_plan"
    bl_label = "刷新站点平面图"

    def execute(self, context):
        invalidate_station_plan(context.scene)
        report = station_plan_report(context.scene.gs_colmap_settings)
        tag_view3d_redraw()
        self.report({"INFO"}, f"预计总站点：{report['total']}")
        return {"FINISHED"}


class GSCOLMAP_OT_render_station_cad_plan(Operator):
    bl_idname = "gs_colmap.render_station_cad_plan"
    bl_label = "重新渲染 CAD 底图"
    bl_description = "按当前场景与路径范围重新生成黑白 CAD 风格平面底图"
        ...
        try:
            ...
            if context.area and context.area.type == "IMAGE_EDITOR":
                ...
                try:
                    ...
                    pass
            tag_view3d_redraw()
            self.report({"INFO"}, f"CAD 底图已更新：{image.size[0]} × {image.size[1]}")
            return {"FINISHED"}
        except Exception as exc:
            self.report({"ERROR"}, str(exc))
            return {"CANCELLED"}
...
class GSCOLMAP_PT_station_plan_image(Panel):
    bl_label = "GS 站点配额平面图"
    bl_idname = "GSCOLMAP_PT_station_plan_image"
    ...
    def poll(cls, context):
        ...
        return bool(settings.station_plan_view_active and image and image.name == STATION_PLAN_IMAGE_NAME)

    def draw(self, context):
        layout = self.layout
        settings = context.scene.gs_colmap_settings
        layout.label(text="CAD 平面底图 + 路径站点", icon="IMAGE_DATA")
        row = layout.row(align=True)
        row.operator("gs_colmap.toggle_station_plan_view", text="返回 3D 视图", icon="LOOP_BACK")
        row.operator("gs_colmap.render_station_cad_plan", text="重渲底图", icon="RENDER_STILL")
        layout.prop(settings, "station_plan_resolution", text="底图分辨率")
        layout.prop(settings, "station_region_default_count", text="新区域站点数")
        layout.operator("gs_colmap.draw_station_region", text="输入数量并在图上拖框", icon="SELECT_SET")
        if settings.station_regions:
            layout.template_list(
                "GSCOLMAP_UL_station_regions", "station_regions_image",
                settings, "station_regions", settings, "station_region_active_index", rows=4,
            )
            index = min(settings.station_region_active_index, len(settings.station_regions) - 1)
            region = settings.station_regions[index]
            layout.prop(region, "station_count", text="所选区域站点数")
            action_row = layout.row(align=True)
            action_row.operator("gs_colmap.remove_station_region", text="删除所选", icon="REMOVE")
            action_row.operator("gs_colmap.clear_station_regions", text="清空全部", icon="TRASH")
        report = station_plan_report(settings)
        layout.label(text=f"当前预计总站点：{report['total']}", icon="LIGHTPROBE_VOLUME")
        for index, (actual, requested) in enumerate(zip(report["regions"], report["requested"])):
            icon = "CHECKMARK" if actual == requested else "INFO"
            layout.label(text=f"区域 {index + 1}：{actual} / 目标 {requested}", icon=icon)
        layout.prop(settings, "station_plan_marker_size", text="站点大小")
        layout.label(text="蓝线=路径 · 青点=常规 · 粉点=框选配额", icon="INFO")
...
class GSCOLMAP_OT_create_path_region(Operator):
    bl_idname='gs_colmap.create_path_region'
    bl_label='在当前视图创建区域框'
    bl_description='在视图中心创建可缩放、旋转和编辑顶点面的线框 Box'
    bl_options={'REGISTER','UNDO'}

    def execute(self,context):
        if context.mode!='OBJECT':
            self.report({'ERROR'},'请先退出编辑模式再创建区域框')
            return {'CANCELLED'}
        settings=context.scene.gs_colmap_settings
        region=context.region_data
        if region is None and context.area and context.area.type=='VIEW_3D':
            region=context.area.spaces.active.region_3d
        location=region.view_location.copy() if region else context.scene.cursor.location.copy()
        size=settings.contour_region_size*_scene_units_per_meter(context.scene)
        bpy.ops.mesh.primitive_cube_add(size=size,location=location)
        obj=context.object
        obj.name='GS_Path_Region'
        settings.contour_region_object=obj
        settings.contour_region_enabled=True
        settings.floorplan_method='CONTOUR'
        return {'FINISHED'}


class GSCOLMAP_OT_edit_path_region(Operator):
    bl_idname='gs_colmap.edit_path_region'
    bl_label='编辑区域形状'
    bl_description='选中区域框并进入编辑模式，可加线、挤出及修改顶点；区域需要保持封闭'
    bl_options={'REGISTER','UNDO'}
    edit: BoolProperty(default=True,options={'HIDDEN'})

    def execute(self,context):
        obj=context.scene.gs_colmap_settings.contour_region_object
        if obj is None or obj.name not in context.view_layer.objects:
            self.report({'ERROR'},'区域框不在当前视图层')
            return {'CANCELLED'}
        ...
        if self.edit:bpy.ops.object.mode_set(mode='EDIT')
        return {'FINISHED'}


class GSCOLMAP_OT_validate_path_region(Operator):
    bl_idname='gs_colmap.validate_path_region'
    bl_label='检查区域封闭性'

    def execute(self,context):
        settings=context.scene.gs_colmap_settings
        try:
            region=region_volume.RegionVolume(settings.contour_region_object,
                context.evaluated_depsgraph_get(),max(1e-9,context.scene.unit_settings.scale_length))
        except Exception as exc:
            self.report({'ERROR'},str(exc))
            return {'CANCELLED'}
        self.report({'INFO'},f'区域 {region.name}：封闭网格检查通过')
        return {'FINISHED'}


def prepare_contour_region(context):
    settings=context.scene.gs_colmap_settings
    if not region_volume.region_enabled(settings):return
    if settings.floorplan_method!='CONTOUR':
        raise ValueError('区域约束仅支持轮廓排线，请切换排线方法')
    obj=settings.contour_region_object
    if obj and obj.mode=='EDIT':
        obj.update_from_editmode()
        bpy.ops.object.mode_set(mode='OBJECT')
    context.view_layer.update()
    region_volume.active_region(context.scene,settings)


class GSCOLMAP_OT_simple_action(Operator):
    bl_idname="gs_colmap.simple_action"
    bl_label="精简流程"
    action: EnumProperty(items=(("PATH","自动生成路径",""),("CAMERAS","生成12方位相机",""),("RENDER","渲染 RGB","")))

    def invoke(self,context,event):
        settings=context.scene.gs_colmap_settings
        settings.live_update_cameras=False
        if self.action=="PATH":
            settings.floorplan_method="CONTOUR";settings.contour_cross_grid_enabled=True
            return bpy.ops.gs_colmap.auto_floorplan_path("INVOKE_DEFAULT")
        if self.action=="CAMERAS":
            settings.rig_mode="PATH";settings.path_capture_mode="LEGACY_PANORAMA_CUBE"
            settings.path_station_array_mode="SPHERICAL_SHELL_12";settings.path_count_mode="COUNT"
            settings.shell_surface_detail_enabled=False
            return bpy.ops.gs_colmap.create_cameras("INVOKE_DEFAULT")
        enforce_simple_outputs(settings);settings.background_render=True
        return bpy.ops.gs_colmap.render_dataset("INVOKE_DEFAULT")
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            if job["process"].poll() is None:
                settings.contour_status, settings.contour_progress = read_contour_progress(job)
                for area in context.screen.areas:
                    area.tag_redraw()
                return {"PASS_THROUGH"}
            result = contour_jobs.finish(job)
            settings.contour_running = False
            settings.contour_progress = 1.0
...
class GSCOLMAP_OT_export_colmap(Operator):
    ...
    bl_label = "Export Sparse Point Cloud Only"
    bl_options = {"REGISTER"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            def point_progress(current, total):
                fraction = current / max(1, total)
                settings.render_progress = fraction
                settings.render_status = f"Sampling sparse point cloud: {current}/{total}"
                window_manager.progress_update(int(fraction * 100))

            window_manager.progress_begin(0, 100)
            try:
                points = sample_sparse_points(context.scene, cameras, settings, point_progress)
            finally:
            ...
            return {"CANCELLED"}
        settings.render_progress = 1.0
        settings.render_status = f"Sparse point cloud exported: {len(points)} points"
...
class GSCOLMAP_PT_coverage_patch(Panel):
    ...
    @classmethod
    def poll(cls, context):
        settings = getattr(context.scene, 'gs_colmap_settings', None)
        return settings is not None and settings.interface_mode == 'PRO'
...
def draw_workflow_progress(layout,settings):
    box=layout.box()
    for key,status,label in (("contour_progress","contour_status","路径生成"),("camera_build_progress","camera_build_status","相机生成"),("render_progress","render_status","渲染")):
        row=box.row();row.enabled=False;row.prop(settings,key,text=label,slider=True)
        text=getattr(settings,status,"")
        if text:box.label(text=text[:110])
    for key,label in (("scientific_planning_progress","科学规划"),("patch_progress","覆盖补拍")):
        if getattr(settings,key,0)>0:
            row=box.row();row.enabled=False;row.prop(settings,key,text=label,slider=True)
    if settings.camera_build_active:box.operator("gs_colmap.cancel_camera_build",text="取消相机生成")
    if settings.contour_running:box.operator("gs_colmap.cancel_contour",text="取消路径生成")
    if _BACKGROUND_RENDER and _BACKGROUND_RENDER["process"].poll() is None:
        box.operator("gs_colmap.cancel_background_render",text="取消渲染")


def draw_simple_panel(layout,context,settings):
    layout.prop(settings,"output_dir",text="输出目录")
    path=layout.box();path.label(text="1 · 自动生成路径")
    path.prop(settings,"floorplan_spacing",text="轮廓间距 (m)")
    path.prop(settings,"contour_cross_grid_spacing",text="井字骨架间距 (m)")
    path.prop(settings,"floorplan_layer_mode",text="路径层数")
    row=path.row();row.enabled=not settings.contour_running
    row.operator("gs_colmap.simple_action",text="生成轮廓 + 井字混合路径",icon="MOD_CURVE").action="PATH"
    camera=layout.box();camera.label(text="2 · 自动排布相机")
    camera.prop(settings,"path_collection",text="路径集合")
    camera.prop(settings,"camera_count",text="站点数量")
    camera.prop(settings,"focal_length",text="焦距 (mm)")
    camera.prop(settings,"camera_overlay_enabled",text="轻量相机代理")
    row=camera.row();row.enabled=not settings.camera_build_active
    row.operator("gs_colmap.simple_action",text="生成 12 方位相机",icon="CAMERA_DATA").action="CAMERAS"
    render=layout.box();render.label(text="3 · RGB 渲染")
    row=render.row(align=True);row.prop(settings,"resolution_x",text="宽");row.prop(settings,"resolution_y",text="高")
    render.prop(settings,"cycles_samples",text="采样数")
    render.prop(settings,"cycles_device",text="渲染设备")
    render.prop(settings,"incremental",text="断点续渲")
    render.operator("gs_colmap.simple_action",text="渲染 RGB 图像",icon="RENDER_STILL").action="RENDER"
    sparse = render.row()
    sparse.operator("gs_colmap.export_colmap", text="仅生成 sparse 点云", icon="POINTCLOUD_POINT")
    render.label(text="保留现有 RGB 图像，不重新渲染", icon="INFO")
...
def draw_gs_colmap_panel(self, context):
    ...
    if not hasattr(scene, "gs_colmap_settings"):
        ...
        return
    settings = scene.gs_colmap_settings
    layout.prop(settings,"interface_mode",expand=True)
    draw_workflow_progress(layout,settings)
    coverage_heat.draw_ui(layout,context,settings)
    if settings.interface_mode=="SIMPLE":
        draw_simple_panel(layout,context,settings)
        return
    incremental_shell = (
        settings.rig_mode == "PATH"
        and settings.path_capture_mode == "LEGACY_PANORAMA_CUBE"
    ...
    try:
        ...
        if settings.rig_mode == "PATH":
            ...
            if settings.path_capture_mode == "LEGACY_PANORAMA_CUBE":
                ...
                if settings.path_station_array_mode == "LEGACY_SIX":
                    ...
                    shell.prop(settings, "shell_surface_clearance")
                    shell.prop(settings, "shell_station_rotation_step")
                    shell.prop(
                        settings, "shell_surface_detail_enabled", text="场景表面补视角", toggle=True,
                        icon="CHECKBOX_HLT" if settings.shell_surface_detail_enabled else "CHECKBOX_DEHLT",
                    )
                    if settings.shell_surface_detail_enabled:
                        detail = shell.box()
                        detail.label(text="依据真实网格遮挡边缘与凹位追加定向相机", icon="MOD_WIREFRAME")
                        detail.prop(settings, "shell_surface_detail_cameras")
                        detail.prop(settings, "shell_surface_probe_distance")
                        detail.prop(settings, "shell_surface_depth_jump")
            ...
            station_box = box.box()
            station_box.enabled = settings.path_capture_mode == "LEGACY_PANORAMA_CUBE"
            station_box.prop(
                settings, "station_plan_enabled",
                text="启用站点配额平面图", toggle=True,
                icon="CHECKBOX_HLT" if settings.station_plan_enabled else "CHECKBOX_DEHLT",
            )
            if settings.path_capture_mode != "LEGACY_PANORAMA_CUBE":
                station_box.label(text="当前仅用于球壳 12 / 旧六面路径站点", icon="INFO")
            if settings.station_plan_enabled:
                row = station_box.row(align=True)
                row.operator(
                    "gs_colmap.toggle_station_plan_view",
                    text="打开中的 CAD 平面图" if settings.station_plan_view_active else "生成并打开 CAD 平面图",
                    icon="LOOP_BACK" if settings.station_plan_view_active else "AXIS_TOP",
                )
                row.operator("gs_colmap.refresh_station_plan", text="刷新", icon="FILE_REFRESH")
                station_box.prop(settings, "station_plan_resolution", text="CAD 底图分辨率")
                station_box.prop(settings, "station_region_default_count", text="新区域站点数")
                if settings.station_regions:
                    station_box.template_list(
                        "GSCOLMAP_UL_station_regions", "station_regions",
                        settings, "station_regions", settings, "station_region_active_index", rows=3,
                    )
                    index = min(settings.station_region_active_index, len(settings.station_regions) - 1)
                    region = settings.station_regions[index]
                    station_box.prop(region, "station_count", text="所选区域站点数")
                    action_row = station_box.row(align=True)
                    action_row.operator("gs_colmap.remove_station_region", text="删除所选", icon="REMOVE")
                    action_row.operator("gs_colmap.clear_station_regions", text="清空全部", icon="TRASH")
                try:
                    plan_report = station_plan_report(settings)
                    station_box.label(text=f"当前预计总站点：{plan_report['total']}", icon="LIGHTPROBE_VOLUME")
                    for index, (actual, requested) in enumerate(zip(plan_report["regions"], plan_report["requested"])):
                        station_box.label(text=f"区域 {index + 1}：{actual} / 目标 {requested}")
                except Exception:
                    station_box.label(text="点击刷新重新计算站点预览", icon="INFO")
                station_box.prop(settings, "station_plan_marker_size", text="站点大小")
                station_box.label(text="区域站点计入总站点预算；超出时按比例缩减", icon="INFO")
                station_box.label(text="进入 CAD 图后直接拖框；区域不可重叠", icon="INFO")
            ...
            if settings.path_capture_mode == "LEGACY_PANORAMA_CUBE":
                ...
                try:
                    ...
                    requested_station_count = (
                        sum(_base_path_component_counts(settings, path_items))
                        if path_items else max(1, settings.camera_count)
                    )
                    station_count = (
                        len(sample_path_components(settings))
                        if settings.station_plan_enabled and path_items
                        else sum(path_component_counts(settings, path_items)) if path_items
                        else max(1, settings.camera_count)
                    )
                except Exception:
                    requested_station_count = max(1, settings.camera_count)
                ...
                maximum_per_station = cameras_per_station
                if settings.path_station_array_mode == "SPHERICAL_SHELL_12":
                    maximum_per_station += _surface_detail_camera_limit(settings)
                budget = box.column(align=True)
                budget.label(text=f"路径站点数: {station_count}")
                if maximum_per_station > cameras_per_station:
                    budget.label(text=f"普通站点: {cameras_per_station} 基础 + 最多 {maximum_per_station - cameras_per_station} 补视角")
                else:
                    budget.label(text=f"每站点相机数: {cameras_per_station}")
                budget.label(
                    text=(
                        f"相机数量上限: {station_count * maximum_per_station}"
                        if maximum_per_station > cameras_per_station
                        else f"预计总相机数: {station_count * cameras_per_station}"
                    ),
                    icon="CAMERA_DATA",
                )
                if any(item['object'].get('gs_peek_route') for item in path_items):
                    budget.label(text="探入短线: 每站 3–12 台定向相机")
                if settings.path_station_array_mode == "SPHERICAL_SHELL_12":
            ...
            if settings.floorplan_expand:
                ...
                if not settings.contour_region_enabled:
                    route_box.prop(settings, "floorplan_space_mode", text="空间范围")
                if settings.floorplan_method == "CONTOUR":
                    region_ui=route_box.column(align=True)
                    region_ui.enabled=not settings.contour_running
                    region_ui.prop(settings,'contour_region_enabled')
                    if settings.contour_region_enabled:
                        region_ui.prop(settings,'contour_region_object')
                        region_ui.prop(settings,'contour_region_size')
                        region_ui.operator('gs_colmap.create_path_region',icon='CUBE')
                        region_actions=region_ui.row(align=True)
                        region_actions.operator('gs_colmap.edit_path_region',text='选中区域',icon='RESTRICT_SELECT_OFF').edit=False
                        region_actions.operator('gs_colmap.edit_path_region',text='编辑形状',icon='EDITMODE_HLT').edit=True
                        region_ui.operator('gs_colmap.validate_path_region',icon='CHECKMARK')
                    route_box.prop(settings, "floorplan_spacing", text="轮廓间距（m）")
                    route_box.prop(settings,"contour_cross_grid_enabled")
                    if settings.contour_cross_grid_enabled:
                        route_box.prop(settings,"contour_cross_grid_spacing")
                        route_box.prop(settings,"contour_cross_grid_budget")
                    ...
                    if settings.floorplan_space_mode != "ALL" and not settings.contour_region_enabled:
                    ...
                    route_box.prop(settings, "contour_peek_enabled")
                    if settings.contour_peek_enabled:
                        route_box.prop(settings, "contour_peek_budget")
                        route_box.prop(settings, "contour_peek_depth")
                    route_box.prop(settings, "contour_camera_audit_enabled")
                    if settings.contour_camera_audit_enabled:
                        route_box.prop(settings, "contour_audit_budget")
                    route_box.operator("gs_colmap.add_doorway_peeks", text="检查现有相机覆盖并补线")
                    route_box.prop(settings, "contour_detail_enabled")
                    route_box.prop(settings, "contour_ceiling_detail_enabled")
                ...
                if settings.contour_running:
                    ...
                    route_box.operator("gs_colmap.auto_floorplan_path", text="在区域内生成路径" if settings.contour_region_enabled else "生成轮廓预览" if settings.floorplan_method == "CONTOUR" else "生成井字排线", icon="MOD_CURVE")
                if settings.floorplan_method == "CONTOUR":
        ...
        inspect_box = box.box()
        inspect_box.label(text="轻量相机代理与路径检查", icon="HIDE_OFF")
        inspect_box.prop(settings, "camera_overlay_enabled", toggle=True)
        if settings.camera_overlay_enabled and not settings.path_inspection_enabled:
            inspect_box.prop(settings, "path_inspection_marker_size")
            inspect_box.prop(settings, "path_inspection_ray_length")
            inspect_box.prop(settings, "path_inspection_xray")
            inspect_box.operator("gs_colmap.refresh_path_inspection", text="刷新全部相机代理")
        if settings.path_inspection_enabled:
            curve = settings.path_inspection_curve
            count = len(path_inspection_cameras(context.scene, curve)) if curve else 0
            inspect_box.label(text=f"路径：{curve.name if curve else '已失效'}")
            inspect_box.label(text=f"轻量代理：{count} 台（青点 + 橙色朝向）")
            row = inspect_box.row(align=True)
            row.operator("gs_colmap.toggle_path_inspection", text="退出并恢复", icon="LOOP_BACK")
            row.operator("gs_colmap.refresh_path_inspection", text="刷新", icon="FILE_REFRESH")
            inspect_box.prop(settings, "path_inspection_marker_size", text="相机点大小")
            inspect_box.prop(settings, "path_inspection_ray_length", text="朝向线长度 (m)")
            inspect_box.prop(settings, "path_inspection_xray", text="穿透显示")
        else:
            inspect_box.label(text="先选中一条路径；完整场景会保留")
            inspect_box.operator(
                "gs_colmap.toggle_path_inspection",
                text="孤立所选路径并查看相机",
                icon="RESTRICT_VIEW_OFF",
            )
        inspect_box.label(text="快捷键：Alt + Shift + I", icon="EVENT_I")
...
from . import coverage_heat

classes = (*coverage_heat.CLASSES,
    GSCOLMAP_StationRegion,
    GSCOLMAP_Settings,
    GSCOLMAP_UL_station_regions,
    GSCOLMAP_OT_create_cameras,
    GSCOLMAP_OT_cancel_camera_build,
    GSCOLMAP_OT_training_recommendations,
    GSCOLMAP_OT_toggle_path_inspection,
    GSCOLMAP_OT_refresh_path_inspection,
    GSCOLMAP_OT_add_doorway_peeks,
    GSCOLMAP_OT_toggle_station_plan_view,
    GSCOLMAP_OT_draw_station_region,
    GSCOLMAP_OT_remove_station_region,
    GSCOLMAP_OT_clear_station_regions,
    GSCOLMAP_OT_refresh_station_plan,
    GSCOLMAP_OT_render_station_cad_plan,
    ...
    GSCOLMAP_OT_simple_action,
    GSCOLMAP_OT_create_path_region,
    GSCOLMAP_OT_edit_path_region,
    GSCOLMAP_OT_validate_path_region,
    ...
    GSCOLMAP_PT_panel,
    GSCOLMAP_PT_station_plan_image,
...
def initialize_registered_scene_state():
    if not hasattr(bpy.types.Scene,'gs_colmap_settings'):
        return None
    if not hasattr(bpy.data,'objects'):
        return .1
    cleanup_legacy_camera_mesh_visuals()
    sync_camera_overlay_on_load()
    return None


def _register_impl():
    global _CAMERA_OVERLAY_HANDLE, _STATION_PLAN_IMAGE_HANDLE
    for cls in classes:
        bpy.utils.register_class(cls)
    bpy.types.Scene.gs_colmap_settings = PointerProperty(type=GSCOLMAP_Settings)
    mesh_guided.register()
    coverage_heat.register()
    bpy.types.TOPBAR_MT_render.append(draw_gs_colmap_render_menu)
    if capture_collision.shutdown not in bpy.app.handlers.load_pre:
        bpy.app.handlers.load_pre.append(capture_collision.shutdown)
    if contour_jobs.shutdown not in bpy.app.handlers.load_pre:
        bpy.app.handlers.load_pre.append(contour_jobs.shutdown)
    if restore_transient_view_modes not in bpy.app.handlers.load_pre:
        bpy.app.handlers.load_pre.append(restore_transient_view_modes)
    if restore_transient_view_modes not in bpy.app.handlers.save_pre:
        bpy.app.handlers.save_pre.append(restore_transient_view_modes)
    if cleanup_legacy_camera_mesh_visuals not in bpy.app.handlers.load_post:
        bpy.app.handlers.load_post.append(cleanup_legacy_camera_mesh_visuals)
    if sync_camera_overlay_on_load not in bpy.app.handlers.load_post:
        bpy.app.handlers.load_post.append(sync_camera_overlay_on_load)
    if hasattr(bpy.data,'objects'):
        initialize_registered_scene_state()
    elif not bpy.app.timers.is_registered(initialize_registered_scene_state):
        bpy.app.timers.register(initialize_registered_scene_state,first_interval=.1)
    if camera_overlay_scene_changed not in bpy.app.handlers.depsgraph_update_post:
        bpy.app.handlers.depsgraph_update_post.append(camera_overlay_scene_changed)
    if not bpy.app.background and _CAMERA_OVERLAY_HANDLE is None:
        _CAMERA_OVERLAY_HANDLE = bpy.types.SpaceView3D.draw_handler_add(
            draw_path_camera_overlay, (), "WINDOW", "POST_VIEW"
        )
    if not bpy.app.background and _STATION_PLAN_IMAGE_HANDLE is None:
        _STATION_PLAN_IMAGE_HANDLE = bpy.types.SpaceImageEditor.draw_handler_add(
            draw_station_plan_image_overlay, (), "WINDOW", "POST_PIXEL"
        )
    if not bpy.app.background:
        ...
        if keyconfig is not None:
            ...
            item = keymap.keymap_items.new(
                "gs_colmap.toggle_path_inspection", "I", "PRESS", alt=True, shift=True
            )
            _PATH_INSPECTION_KEYMAPS.append((keymap, item))



def register():
    # A failed earlier install may leave just the first few classes registered.
    addon_lifecycle.cleanup(globals())
    try:
        _register_impl()
    except Exception:
        addon_lifecycle.cleanup(globals())

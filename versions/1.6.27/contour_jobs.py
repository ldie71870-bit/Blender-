...
def _is_windows_file_lock(error):
    """Whether Windows temporarily denied replacing a file held by another process."""
    return isinstance(error, PermissionError) or getattr(error, 'winerror', None) in (5, 32)


def write_json(path, payload, required=True):
    """Atomically write a worker side-channel file.

    The foreground Blender process reads ``progress.json`` while the worker
    updates it. Windows may temporarily lock that file, so optional progress
    writes retry and may be skipped. The final result is still required: a
    failure writing it must remain visible to the job supervisor.
    """
    path = Path(path)
    ...
    try:
        for attempt in range(12):
            try:
                # A fixed ``progress.tmp`` can itself remain locked after a
                # previous failed replacement. Use a fresh sibling each time.
                ...
                if temporary is not None:
                    try:
                        ...
                        pass
                    temporary = None
                if not _is_windows_file_lock(error) or attempt == 11:
                    if required:
                        raise
                    return False
                time.sleep(min(.02 * (attempt + 1), .08))
    finally:
        if temporary is not None:
            try:
                temporary.unlink(missing_ok=True)
    ...
    try:
        ...
        def progress(stage, current, total):
            ...
            if now-last[0] > .5 or current in (0,total):
                if write_json(root/'progress.json',dict(stage=stage,current=current,total=total),
                              required=False):
                    last[0] = now
...
def fraction(job):
    try:
        data=json.loads((job['root']/'progress.json').read_text(encoding='utf8'))
        stage=data.get('stage','')
        buckets=[('建立',.02,.05),('地面',.07,.18),('层',.25,.12),('轮廓',.37,.1),('井字',.47,.08),('表面',.55,.12),('细部',.67,.1),('门洞',.77,.08),('相机',.85,.1)]
        base,span=.05,.02
        for word,b,w in buckets:
            if word in stage:base,span=b,w;break
        value=base+span*max(0.,min(1.,float(data.get('current',0))/max(1.,float(data.get('total',1)))))
        job['fraction']=max(job.get('fraction',0.),min(.97,value))
    except (OSError,ValueError,TypeError):pass
    return job.get('fraction',0.)

...
def audit_and_refine(scene,routes,probe,cameras,budget=16,progress=None):
    ...
    for inst in probe.depsgraph.object_instances:
        obj=inst.object
        if obj.type!='MESH' or region_volume.is_region(obj) or obj.get('gs_contour_version') or obj.get('gs_camera_mesh_visual'):continue
        ...
        try:
            ...
            for face in list(mesh.polygons)[::stride][:20]:
                ...
                if probe.region and not probe.region.contains(p):continue

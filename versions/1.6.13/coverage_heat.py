...
def changed(scene,depsgraph):
    ...
    for update in depsgraph.updates:
        relevant=isinstance(update.id,(bpy.types.Mesh,bpy.types.Camera)) or (isinstance(update.id,bpy.types.Object) and update.id.type in {'MESH','CAMERA'} and (update.is_updated_geometry or update.is_updated_transform))
        if relevant:
            REVISIONS[scene.as_pointer()]=REVISIONS.get(scene.as_pointer(),0)+1
            if st:
                st['dirty']=True;scene.gs_colmap_settings.coverage_status='场景已变化，请重新检测覆盖'
            return
...
def repair(scene,settings,st):
    ...
    try:
        ...
        raise
    if selected:st['dirty']=True
    yield 1.,f'新增 {len(created)} 台正式补拍相机；请重新检测查看颜色变化' if created else '没有满足净空、重叠和新增视差的补拍位置；未创建无效相机'
...
class GS_OT_heat_job(bpy.types.Operator):
    ...
    def start(self,context):
        ...
        self.iterator=analysis(self.scene,s) if self.action=='ANALYZE' else repair(self.scene,s,state(self.scene))
...
class GS_OT_coverage_brush(bpy.types.Operator):
    ...
    def invoke(self,context,event):
        ...
        self.area.header_text_set('左键刷选 · Shift 左键擦除 · 中键导航 · Enter / Esc 完成')
    ...
    def modal(self,context,event):
        ...
        if event.type=='LEFTMOUSE':self.dragging=event.value=='PRESS'
...
class GS_OT_clear_brush(bpy.types.Operator):
    bl_idname='gs_colmap.clear_coverage_brush';bl_label='清空刷选'
    def execute(self,context):
        st=state(context.scene)
        if st:st['selected'].clear();st['batch']=None
        redraw();return {'FINISHED'}

CLASSES=(GS_OT_heat_job,GS_OT_coverage_brush,GS_OT_clear_brush)
...
def draw_ui(layout,context,settings):
    ...
    row.operator('gs_colmap.coverage_brush',text='开始刷选');row.operator('gs_colmap.clear_coverage_brush',text='清空')

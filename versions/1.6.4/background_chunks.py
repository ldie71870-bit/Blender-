...
def detect_fatal_log(text, backend="AUTO"):
    """Return fatal signatures relevant to the backend used by this worker.

    Blender can probe optional Cycles backends during startup and print warnings for a
    backend that is not active. For example, HIP workers commonly log an OptiX init warning
    on machines that have an NVIDIA runtime component installed. Treating that warning as a
    failed render caused successful HIP chunks to be retried forever.
    """
    signatures = [name for name, pattern in FATAL_LOG_PATTERNS.items() if pattern.search(text or "")]
    active = str(backend or "AUTO").upper()
    if active == "AUTO":
        match = re.search(r"Cycles GPU active:\s*backend=([A-Za-z0-9_]+)", text or "")
        if match:
            active = match.group(1).upper()
    if active not in {"OPTIX", "AUTO"}:
        signatures = [name for name in signatures if name != "optix"]
    if active not in {"CUDA", "OPTIX", "AUTO"}:
        signatures = [name for name in signatures if name != "cuda"]
    if "HIP_OOM_RECOVERED_CPU" in (text or ""):
        signatures = [name for name in signatures if name != "out_of_memory"]
    return signatures
...
def _run_worker(config, requested_start, attempt):
    ...
    fatal_signatures = detect_fatal_log(text, config.get("cycles_backend", "AUTO"))
    ...
    return {
        ...
        "cycles_backend": str(result.get("cycles_backend", config.get("cycles_backend", "AUTO"))),
        "requested_cycles_backend": str(config.get("cycles_backend", "AUTO")),
        "hip_oom_recovered": bool(result.get("hip_oom_recovered", False)),
...
def run_supervisor(config_path):
    ...
    history = {
        ...
        "cycles_backend": str(config.get("cycles_backend", "AUTO")),
        "hip_memory_safe_mode": bool(config.get("hip_memory_safe_mode", False)),
        "retry_limit": 1,
    ...
    while True:
        ...
        for attempt in (1, 2):
            ...
            if attempt == 1:
                if (
                    "out_of_memory" in record["fatal_signatures"]
                    and int(record.get("chunk_size", 1)) <= 1
                ):
                    print(
                        "[GS] HIP OOM occurred while rendering one frame; not retrying the same "
                        "single-frame allocation. Inspect the HIP-RT and render-preflight lines above.",
                        flush=True,
                    )
                    break
                if "out_of_memory" in record["fatal_signatures"]:
                    previous_size = max(1, int(config.get("chunk_size", 1)))
                    reduced_size = max(1, previous_size // 2)
                    if record.get("cycles_backend") == "HIP" and config.get("hip_memory_safe_mode"):
                        reduced_size = min(
                            reduced_size,
                            max(1, int(config.get("hip_chunk_size", 1))),
                        )
                    config["chunk_size"] = reduced_size
                    print(
                        f"[GS] OOM detected; reducing {record.get('cycles_backend', 'AUTO')} "
                        f"chunk size {previous_size} -> {reduced_size} before retry",
                        flush=True,
                    )

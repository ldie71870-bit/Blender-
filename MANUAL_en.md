...
7. Click `Create Camera Rig` and inspect the green pyramid directions.
...
## Green Pyramid Camera Style

Generated cameras are shown in the 3D View as green pyramid meshes. The apex is the optical centre and the square base lies along local `-Z`, which is the lens direction.

The pyramid is only a viewport proxy. The real object remains a Blender `CAMERA` and continues to drive Cycles rendering, depth and ID passes, COLMAP poses, and `transforms.json`. Proxies live in `GS_CAMERA_MESH_STYLE` and are excluded from rendering, scientific planning, sparse point sampling, and object groups.

Use `Convert Scene Cameras` for all existing cameras, `Convert Selected` for a selection, and `Remove Mesh Appearance` to remove only the proxies. `Pyramid Size (m)` changes display size without changing focal length or field of view.
...
Cycles Persistent Data: enabled by default to reuse Cycles BVH, textures, and render data inside one chunk. Every chunk still exits its worker process and releases all caches. Disable it for memory-constrained scenes.
